#!/usr/bin/env python3
"""DuckDB bridge generator: UCC canonical lenders × SBA lenders (Lance edition).

Cycle: ucc-gleif-identity-spine (s5).

Filter-layer bridge. Both sides ALREADY have pre-normalized name columns.
No inline normalization needed.

Reads:
  UCC: polaris-warehouse/ucc_ca/lenders_lance/  (101K canonical UCC lenders)
...
Output: polaris-warehouse/bridges/ucc_sba_lender_lance/
Floor: ≥ 1,000 rows.

Usage:
  doppler run --project hq-all --config prd -- \\
    uv run --with duckdb --with pylance --with pyarrow --with "psycopg[binary]" python \\
    apps/data-engine-x/scripts/build_bridge_ucc_sba_lender_lance.py --apply
"""
...
logger = logging.getLogger("build_bridge_ucc_sba_lender_lance")

BRIDGE_NAME = "ucc_sba_lender"
...
UCC_LENDERS_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/ucc_ca/lenders_lance"
SBA_LENDERS_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/sba/lenders_lance"
BRIDGE_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/ucc_sba_lender_lance"
DATASET_SLUG = "ucc_sba_lender_lance"
...
def _materialize_inputs(storage_options: dict) -> tuple:
    ...
    logger.info("  ucc_ca/lenders_lance: %d rows", rows_left)
...
def _ensure_registry() -> None:
    register_match_method(
        ...
        description=(
            "Exact-equality JOIN on entity_name_normalized (no state filter — "
            "lenders operate nationally). Applies pre-normalized name columns."
        ),
    )
    register_match_method_version(
        method_name=METHOD_NAME,
        ...
        input_columns_left=["lender_name_normalized"],
        input_columns_right=["bankname_normalized"],
        output_value_description="normalized lender name join key",
    )
    register_bridge(
        ...
        description=(
            "UCC CA canonical lenders × SBA originating lenders. "
...
def main() -> int:
    ...
    logger.info("inputs: UCC lenders_lance + SBA lenders_lance (Arrow-bridge)")
