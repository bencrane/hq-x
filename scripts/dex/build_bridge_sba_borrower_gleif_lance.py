#!/usr/bin/env python3
"""DuckDB bridge generator: SBA borrowers × GLEIF LEI records (Lance edition).

Cycle: ucc-gleif-identity-spine (s8).
...
Join: sba.legal_name_normalized = gleif.legal_name_normalized
      AND upper(sba.borrstate) = substr(gleif.headquarters_region, 4, 2)
...
Expected LOW coverage (<1% of SBA borrowers are LEI-registered) — ground-truth
anchor for future fuzzy-match calibration.

Output: polaris-warehouse/bridges/sba_borrower_gleif_lance/
Floor: ≥ 10,000 rows.

Usage:
  doppler run --project hq-all --config prd -- \\
    uv run --with duckdb --with pylance --with pyarrow --with "psycopg[binary]" python \\
    apps/data-engine-x/scripts/build_bridge_sba_borrower_gleif_lance.py --apply
"""
...
logger = logging.getLogger("build_bridge_sba_borrower_gleif_lance")

BRIDGE_NAME = "sba_borrower_gleif"
...
BRIDGE_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/sba_borrower_gleif_lance"
DATASET_SLUG = "sba_borrower_gleif_lance"
...
def _materialize_inputs(storage_options: dict) -> tuple:
    ...
    logger.info("  sba/borrowers_lance: %d rows", rows_left)
...
def _build_match_table(
    ...
    *,
    ...
    # Fan-out on name alone — state removed (permissive; GLEIF registered entities national)
    con.execute("""
        CREATE TEMP TABLE sba_fanout AS
        SELECT legal_name_normalized AS norm_name,
               COUNT(*) AS sba_fan_out
        FROM sba_clean GROUP BY legal_name_normalized
    """)
    con.execute("""
    ...
    con.execute(
        ...
        FROM sba_clean s
        JOIN gleif_clean g
          ON g.legal_name_normalized = s.legal_name_normalized
        JOIN sba_fanout sf
          ON sf.norm_name = s.legal_name_normalized
...
def _ensure_registry() -> None:
    register_match_method(
        ...
        description=(
            ...
            "Both sides pre-normalized. GLEIF state from substr(headquarters_region, 4, 2)."
        ),
    )
    register_match_method_version(
        method_name=METHOD_NAME,
    ...
    register_bridge(
        ...
        description=(
            "SBA borrowers × GLEIF LEI records. Low coverage (<1%) but high-signal: "
            "ground-truth anchor for future fuzzy-match calibration. Both sides pre-normalized."
        ),
    )
...
def main() -> int:
    ...
    logger.info("inputs: SBA borrowers_lance + GLEIF lei_records_lance (Arrow-bridge)")
