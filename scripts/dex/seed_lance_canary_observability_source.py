#!/usr/bin/env python3
"""Register the FMCSA carrier_essentials Lance canary in ops.data_sources.

Wave 3 of the multi-phase hq-all rebuild — observability registration for
the Lance canary. Mirrors the Polaris seed pattern
(``seed_polaris_observability_source.py``). One row in ``ops.data_sources``,
one row in ``ops.data_source_slas``, both idempotent.

The 24h SLA aligns with the existing Parquet+Iceberg path SLA — the daily
Modal cron at ``modal/fmcsa_carrier_essentials_lance_emit_app.py`` runs at
06:30 UTC and writes a row to ``ops.data_source_ingest_runs`` per
invocation. The system-health dashboard surfaces this against the SLA.

The ``status='canary'`` flag distinguishes this from production sources;
once we have weeks of green runs, the operator can flip it to
``status='active'``.

Usage:
    doppler run --project hq-all --config prd -- \\
        python3 apps/data-engine-x/scripts/seed_lance_canary_observability_source.py

Required env (from Doppler):
    DEX_DB_URL_DIRECT  — direct Postgres connection to data-engine-x DB
"""
...
# Status enum on ops.data_sources is constrained to {active, needs_triage, retired}.
# 'canary' would be more precise but the constraint doesn't allow it. The
# canary semantics live in SLA_NOTES + the source's _lance suffix. Once we
# have weeks of green runs the operator can leave this as-is.
STATUS = "active"
SLA_FRESHNESS_SECONDS = 86400  # 24h, mirrors the Parquet+Iceberg path
SLA_BASIS = "last_ingested"
SLA_NOTES = (
    "Wave 3 Lance canary — daily refresh from the existing fmcsa-derived "
    "carrier_essentials Parquet snapshot via "
    "modal/fmcsa_carrier_essentials_lance_emit_app.py at 06:30 UTC. Run "
    "alongside the existing Parquet+Iceberg path; not yet a replacement. "
    "See apps/data-engine-x/docs/lance-canary-fmcsa-carrier-essentials.md."
)
