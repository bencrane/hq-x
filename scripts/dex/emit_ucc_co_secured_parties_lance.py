#!/usr/bin/env python3
"""s3 — Emit UCC CO secured parties as a Lance dataset on R2.

Reads:  s3://dex-raw-landing-zone/ucc/state=CO/stream=secured_parties/snapshot=2026-05-08/data.parquet
Writes: s3://dex-raw-landing-zone/polaris-warehouse/ucc_co/secured_parties_lance/

Column rename map (CO source → CA parity schema, 15 output columns):
  fileid            → UCC1_NUM
  (none)            → UCC3_NUM               (NULL — CO has no UCC-3 amendment num)
  (derived)         → SECURED_PARTY_TYPE     ('Organization' if organizationname not null, else 'Individual')
...
CO-only columns (party_name_normalized, assignor, etc.) are intentionally dropped.
The SECURED_PARTY_TYPE derivation MUST use the same CASE expression as CA's schema so
that s5 (build_ucc_co_lenders_lance.py) can filter on SECURED_PARTY_TYPE = 'Organization'.

BTREE index on UCC1_NUM (join key back to filings).
Commit-lock via Postgres advisory lock (lance_commit_lock constraint).

MIN_ROW_FLOOR = 1_950_000 — HARD FAIL gate fires BEFORE lance.write_dataset().
Source has 2,055,777 rows; floor at ~94.9%.

Usage:
    doppler run --project hq-all --config prd -- \\
        uv run --quiet python3 apps/data-engine-x/scripts/emit_ucc_co_secured_parties_lance.py \\
            --apply
"""
...
PARQUET_URI = (
    "r2://dex-raw-landing-zone/ucc/state=CO/stream=secured_parties/snapshot=2026-05-08/data.parquet"
)
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Emit UCC CO secured parties Lance (s3)")
    ...
    with lance_commit_lock(DATASET_SLUG):
        ...
        # NOTE: SECURED_PARTY_TYPE derivation mirrors CA convention exactly so that
        # build_ucc_co_lenders_lance.py can filter on SECURED_PARTY_TYPE = 'Organization'
        reader = con.execute(
            f"""
            SELECT
                fileid AS UCC1_NUM,
                CAST(NULL AS VARCHAR) AS UCC3_NUM,
                CASE WHEN organizationname IS NOT NULL THEN 'Organization' ELSE 'Individual' END AS SECURED_PARTY_TYPE,
