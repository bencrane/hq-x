#!/usr/bin/env python3
"""Project FMCSA Vehicle Inspection Parquet → derived essentials Parquet on R2.

Reads `fmcsa/Vehicle Inspection File/<snapshot>/*.parquet.zst` via DuckDB-on-R2,
projects DOT_NUMBER + temporal + violation/OOS counts, writes plain `.parquet`
(internal column ZSTD only) to:

    s3://dex-raw-landing-zone/fmcsa-derived/vehicle_inspection_essentials/snapshot=<YYYY-MM-DD>/data.parquet

Per-run audit row written to ops.fmcsa_derived_vehicle_inspection_r2_ingest_runs.

L40 closure for FMCSA Vehicle Inspection feed.
...
Usage:
    doppler run -p hq-all -c prd -- \\
        ...
        apps/data-engine-x/scripts/build_fmcsa_vehicle_inspection_essentials.py --apply
"""
...
logger = logging.getLogger("build_fmcsa_vehicle_inspection_essentials")

R2_BUCKET = "dex-raw-landing-zone"
INPUT_PREFIX = "fmcsa/Vehicle Inspection File"
OUTPUT_PREFIX = "fmcsa-derived/vehicle_inspection_essentials"
AUDIT_TABLE = "ops.fmcsa_derived_vehicle_inspection_r2_ingest_runs"

# Per-feed projection. DOT_NUMBER (L14 identity) + INSP_DATE for L37
# static-date filter. Counts (VIOL_TOTAL / OOS_TOTAL / DRIVER_* / VEHICLE_*)
# drive the OOS-rate signal in MV #20.
ESSENTIALS_COLS = [
    "DOT_NUMBER",
    "INSPECTION_ID",
    "INSP_DATE",
            ...
            cur.execute(
                ...
                INSERT INTO {AUDIT_TABLE}
                  (snapshot_date, upstream_input_prefix, output_r2_prefix,
                   status, triggered_by)
                VALUES (%s, %s, %s, 'running', 'manual:build_fmcsa_vehicle_inspection_essentials')
                RETURNING run_id;
                """,
