#!/usr/bin/env python3
"""FMCSA daily-delta event-stream feeds → RisingWave source wiring (Stage 4).

Wires 6 FMCSA event-stream R2 prefixes as RisingWave ``CREATE SOURCE ...
FORMAT PLAIN ENCODE PARQUET`` catalog entries. Sources are metadata-only —
RW does not read any Parquet until an MV consumes from the source. Cost:
near-zero CU.

Sources wired by this applier:

  source_fmcsa_out_of_service_orders   (7 cols)
  source_fmcsa_insurance               (9 cols)
  source_fmcsa_inshist                 (17 cols)
  source_fmcsa_carrier                 (43 cols)
  source_fmcsa_actpendinsur            (11 cols)
  source_fmcsa_rejected                (15 cols)

Per-source flow:

  1. R2 ground-truth (L8): boto3 list_objects_v2 + fnmatch over the
     stream's match_pattern; require ≥ 1 matching object.
  2. DuckDB-on-R2 ``DESCRIBE`` introspection on ONE sample file. Captures
     (col_name, duckdb_type) pairs. The directive's all-VARCHAR
     assumption is verified: every introspected col must be DuckDB
     VARCHAR or the applier hard-fails (the directive's premise needs
     validation before any DDL ships).
  3. Build CREATE SOURCE DDL — every column maps to ``CHARACTER VARYING``
     per L2 path #1.
  4. Idempotency: ``SELECT 1 FROM rw_catalog.rw_sources WHERE name = ?``
     — if hit, skip.
  5. DDL apply via psycopg.
  6. Smoke (per L6): ``SELECT name FROM rw_catalog.rw_sources WHERE name
     = ?`` returns 1 row. Do NOT run ``SELECT count(*)`` — that
     triggers eager hydration. Per L2, no typed-column smoke is needed
     for this sweep — every column is VARCHAR.
  7. Insert ledger row into ``ops.rw_source_wiring_runs`` (REUSED from
     PR #249).
...
2026-05-10-rw-source-sweep-fmcsa-6-event-stream-feeds):
  ...
  - L2: every introspected column is VARCHAR; DDL declares CHARACTER
        VARYING. No typed-column footgun in this sweep.
  - L4: ledger CHECK includes {completed, failed, skipped, dry_run}.
  - L6: smoke = rw_catalog.rw_sources only. No count(*).
  - L7: column names with spaces / special chars are double-quoted.
  - L8: R2 ground-truth before DDL; fail-loud if 0 matches.
  - L40: NON-APPLICABLE — files are PAR1 plain Parquet with internal
         column-chunk ZSTD; RW reads natively. No derivation needed.
...
Usage:
  doppler run --project hq-all --config prd -- \\
      uv run python3 scripts/apply_rw_source_sweep_fmcsa_event_streams.py --apply
  doppler run --project hq-all --config prd -- \\
      uv run python3 scripts/apply_rw_source_sweep_fmcsa_event_streams.py --dry-run
  doppler run --project hq-all --config prd -- \\
      uv run python3 scripts/apply_rw_source_sweep_fmcsa_event_streams.py \\
          --source source_fmcsa_insurance --apply
  doppler run --project hq-all --config prd -- \\
      uv run python3 scripts/apply_rw_source_sweep_fmcsa_event_streams.py --smoke-only

Per-source debug budget: 5 min wall-clock. Exceeding it logs ``failed``
...
DIRECTIVE_SLUG = "2026-05-10-rw-source-sweep-fmcsa-6-event-stream-feeds"
...
SOURCES: tuple[SourceSpec, ...] = (
    SourceSpec(
        "source_fmcsa_out_of_service_orders",
        "fmcsa_event_stream",
        "fmcsa/OUT OF SERVICE ORDERS/*/*.parquet.zst",
    ),
    SourceSpec(
        "source_fmcsa_insurance",
        "fmcsa_event_stream",
        "fmcsa/Insurance/*/*.parquet.zst",
    ),
    SourceSpec(
        "source_fmcsa_inshist",
        "fmcsa_event_stream",
        "fmcsa/InsHist/*/*.parquet.zst",
    ),
    SourceSpec(
        "source_fmcsa_carrier",
        "fmcsa_event_stream",
        "fmcsa/Carrier/*/*.parquet.zst",
    ),
    SourceSpec(
        "source_fmcsa_actpendinsur",
        "fmcsa_event_stream",
        "fmcsa/ActPendInsur/*/*.parquet.zst",
    ),
    SourceSpec(
        "source_fmcsa_rejected",
        "fmcsa_event_stream",
        "fmcsa/Rejected/*/*.parquet.zst",
    ),
)
...
def _logger() -> logging.Logger:
    ...
    return logging.getLogger("apply-fmcsa-event-streams-rw-sources")
...
def introspect_columns(
    ...
    ``hive_partitioning=FALSE`` because the directory layout uses
    ``YYYY-MM-DD/`` (no key=value Hive convention), but DuckDB's auto
    Hive detection occasionally produces spurious partition columns —
    explicit FALSE is safest.
    """
    rows = duck.execute(
...
def process_source(
    *,
    ...
    # ---- Smoke-only: source must exist; rw_catalog.rw_sources visibility check. ----
    if mode == "smoke_only":
    ...
    try:
        ...
        sample_key = all_keys[-1]  # latest snapshot — most representative
        log.info(
        ...
        # L2 verification: every column must be VARCHAR per directive's
        # all-VARCHAR assumption. Non-VARCHAR contradicts the directive
        # premise — hard-fail and surface to operator rather than papering over.
        ...
        # Smoke (per L6) — rw_catalog.rw_sources visibility, no count(*).
        if not source_exists_in_rw(rw, entry.name):
            raise RuntimeError(
