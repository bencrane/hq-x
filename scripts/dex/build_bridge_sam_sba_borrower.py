#!/usr/bin/env python3
"""DuckDB bridge generator: SAM-registered entity × SBA borrower (Lance edition).

*** REWRITTEN 2026-05-12 — inputs + output now read/write Lance datasets ***
*** Previous version read raw SAM parquets + SBA raw parquets, wrote parquet ***
*** This version reads Lance and writes Lance per the sba-bridges-to-lance cycle ***

Companion to `build_bridge_pdl_sba_borrower.py` (Lance edition). SAM is the
federal-contractor registry. Where SAM and PDL both match an SBA borrower,
SAM is AUTHORITATIVE because:
  - `entity_url` is government-validated
  - Carries strategic signals: `bus_type_string` (8(a)/HUBZone/WOSB/etc.),
    `govt_bus_poc_*`, `unique_entity_id` (cross-source identity key)

Reads:
  SAM: `polaris-warehouse/sam_gov/entities_lance/` (written by emit_sam_entities_lance.py)
...
Measured 10.9% divergence vs the SBA-style normalizer on a 200-row sample.
Re-normalize from raw `legal_business_name` using the SBA-style normalizer.
...
Output: `polaris-warehouse/bridges/sam_sba_borrower_lance/`

Row floor: ≥ 360,000 (per directive §"Volume floors"; tightened from 30K).

Usage:
  doppler run --project hq-all --config prd -- \\
    uv run --with duckdb --with pylance --with pyarrow --with "psycopg[binary]" python \\
    apps/data-engine-x/scripts/build_bridge_sam_sba_borrower.py --apply

  doppler run --project hq-all --config prd -- \\
    uv run --with duckdb --with pylance --with pyarrow --with "psycopg[binary]" python \\
    apps/data-engine-x/scripts/build_bridge_sam_sba_borrower.py --dry-run
"""
...
logger = logging.getLogger("build_bridge_sam_sba_borrower")

# Bridge identity
BRIDGE_NAME = "sam_sba_borrower"
...
BRIDGE_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/sam_sba_borrower_lance/"
DATASET_SLUG = "sam_sba_borrower_lance"

# Tier thresholds
COLLISION_THRESHOLD = 50

# Row floor — borrower-level pairs (new Lance bridge).
# Old parquet bridge: 384K rows at loan-level (377K distinct loans, 227K distinct names).
# New Lance bridge dedupes to borrower granularity; floor adjusted down.
...
def _normalize_entity_sql(raw_expr: str) -> str:
    """SQL equivalent of entity_name_normalize.py v1.0.0. NORMALIZER_VERSION={NORMALIZER_VERSION}

    CRITICAL: do NOT use SAM's pre-computed legal_business_name_normalized —
    measured 10.9% divergence vs the SBA-style normalizer on a 200-row sample.
    Re-normalize from raw legal_business_name using this formula.
    """
...
def _materialize_inputs(storage_options: dict) -> tuple:
    """Read Lance datasets via Arrow-bridge; return (sam_arrow, sba_arrow, rows_l, rows_r)."""
    import lance
    ...
    # Scan only the columns needed for the join + bridge output
    sam_cols = [
    ...
    # Some columns may not exist in all snapshots — scan tolerates missing cols
    available_cols = [f.name for f in sam_ds.schema]
    scan_cols = [c for c in sam_cols if c in available_cols]
    ...
    return sam_arrow, sba_arrow, rows_left, rows_right
...
def _build_match_table(
    ...
    *,
    ...
    """Compute fan-out tiers + JOIN; return (con, counts_dict)."""
    import duckdb
    ...
    # Build sam_branded with re-normalized name (NOT the pre-computed field)
    con.execute(
        f"""
    ...
    con.execute(
        ...
        FROM sam_branded s
        JOIN sba_clean b
          ON b.legal_name_normalized = s.sam_name_normalized
         AND b.borrstate = s.sam_state
...
def _write_bridge_lance(con, storage_options: dict) -> int:
    """Write bridge_match to Lance; return row count."""
    import lance
...
def _ensure_registry() -> None:
    ...
    register_bridge(
        ...
        description=(
            "SAM entities × SBA borrower (Lance edition). "
            "Provides entity_url, cage_code, bus_type_string (8(a)/HUBZone/WOSB), "
            "govt_bus_poc contacts. Lance-rewrite of the 2026-05-10 parquet bridge."
        ),
    )
...
def main() -> int:
    ...
    logger.info("inputs: SAM entities Lance + SBA borrowers Lance (Arrow-bridge)")
    ...
    try:
        sam_arrow, sba_arrow, rows_left, rows_right = _materialize_inputs(storage_options)
