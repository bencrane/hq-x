"""OR CCB Active Licenses -> Lance emit (Pattern A, one-off operator-fired).

Reads the Oregon Construction Contractors Board (CCB) "Active Licenses" registry
(Socrata data.oregon.gov / g77e-6bhs) and writes a Lance dataset at:
    s3://dex-raw-landing-zone/polaris-warehouse/licensure/or_ccb_lance

R2 Parquet intermediate at:
    s3://dex-raw-landing-zone/or-ccb/release={release}/data.parquet

Source: https://data.oregon.gov/resource/g77e-6bhs.csv (clean Socrata CSV;
field-name header, no title row). 55,841 rows confirmed 2026-05-29.

Columns:
  - 24 verbatim from CSV (all VARCHAR per L9 "emit verbatim"):
      license_number, license_type, related_key, related_type, county_code,
      county_name, lic_exp_date, orig_regis_date, bond_company, bond_amount,
      bond_exp_date, ins_company, ins_amount, ins_exp_date, full_name, address,
      city, state, zip_code, phone_number, fax_number, rmi_name, exempt_text,
      endorsement_text
  - business_name_normalized (additive, via normalize_entity_name UDF per L34).

BTREE indices on license_number + business_name_normalized + license_type
(RGC/CGC1/CGC2 = general contractors; RSC/CSC* = specialty -- the GC filter).
Mirrors run_wa_lni_contractors_lance_emit.py.
...
Run via:
    cd ~/hq-all/apps/data-engine-x && \\
      ...
      uv run python scripts/run_or_ccb_lance_emit.py \\
        --csv-path "/tmp/or_ccb.csv" --release 2026-05-29
"""
...
DATASET_SLUG = "or_ccb_lance"
LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/licensure/or_ccb_lance"
MIN_ROW_FLOOR = 50_000  # probe parsed 55,841 on 2026-05-29
...
def emit(csv_path: str, release: str) -> dict:
    ...
    local_parquet = f"/tmp/or-ccb-{release}.parquet"
    r2_key = f"or-ccb/release={release}/data.parquet"
    ...
    con.execute(
        f"""
        COPY (
            SELECT *, py_normalize_entity("full_name") AS business_name_normalized
    ...
    for col in ("license_number", "business_name_normalized", "license_type"):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="OR CCB Active Licenses -> Lance emit (one-off)")
