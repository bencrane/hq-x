"""FMCSA CO carrier × CO UCC-1 debtor Pattern B Lance bridge.

Pattern B exact-match bridge: FMCSA motor carriers registered in Colorado
(fmcsa/carrier_essentials_lance, phy_state='CO') × CO UCC-1 debtor filings
(ucc_co/debtors_lance, DEBTOR_TYPE='Organization', STATE='CO'), joined on
normalized legal_name. Payoff: identify which CO motor carriers carry UCC liens
(equipment loans, asset-based lending, factoring) — a capital / lender-intelligence
layer over the CO carrier base.

CO counterpart of build_bridge_fmcsa_ucc_ca_debtor_lance.py (shipped PR #619,
2026-05-21). Identical FMCSA side; UCC side swapped from ucc_ca/debtors_lance
to ucc_co/debtors_lance and state filters swapped CA -> CO.

Method: legal_name_state_exact_co v1.0.0 (REUSED — the method + version rows
were registered by build_bridge_ppp_ucc_co_debtor_lance.py (PR #615); reused by
sam_ucc_co_debtor (#616) and usaspending_ucc_co_debtor (#617). This script ONLY
calls register_bridge + start_bridge_run + complete_bridge_run + fail_bridge_run
— the method-definition and method-version-definition helpers are INTENTIONALLY
OMITTED. Calling register_match_method / register_match_method_version would
UPSERT over the shared match_method_versions row and corrupt the provenance trail
of the three existing consumers: ppp_ucc_co_debtor, sam_ucc_co_debtor,
usaspending_ucc_co_debtor. Mirror: build_bridge_sam_ucc_co_debtor_lance.py
(a REUSER, not the DEFINER).

FMCSA side:
  - scanner filter: phy_state='CO' (validator-confirmed lowercase columns;
    value is the literal string 'CO'). 79,736 CO carriers.
...
UCC debtor side (validator-confirmed schema — 15-column CA-parity):
  - scanner filter: DEBTOR_TYPE='Organization' (validator-confirmed live PyLance scan
    2026-05-21: DEBTOR_TYPE distinct set is exactly {'Organization': 938312,
    'Individual': 1047589} — same literal as the CA dataset; C8 org-to-org only)
    AND STATE='CO' (structural no-op — every ucc_co/debtors_lance row is 'CO';
    kept for CA-template parity and explicitness — directive C8).
  ...
  - Filing-grain (no SELECT DISTINCT) — mirrors CA template fidelity; the UCC
    filing-level detail (UCC1_NUM, UCC3_NUM, address) is meaningful downstream.
...
Normalizer discipline (C3):
  ...
  faithfully inlined as SQL). Canonical reference: the CA counterpart and the
  SAM × CO UCC bridge.

Fan-out (symmetric two-sided, validator-fixed formula):
  ...
  and collapses the gold tier to 0.
...
MIN_ROWS_MATCHED = 73_000 (validator-calibrated 2026-05-21 — ~54.4% of measured
    est_rows_matched=134,196; conservative end of 55-60% band because name+state
    matching is fuzzier than domain/UEI joins. Hard fail if below floor — C9.)

Validator-measured estimate (benchmark run 3x, deterministic 0% stddev):
    rows_matched=134,196 (platinum=9,024; gold=104,207; silver=20,965; rejected=38,220)

Inputs:
    s3://dex-raw-landing-zone/polaris-warehouse/fmcsa/carrier_essentials_lance
        (filter phy_state='CO'; normalize legal_name via _lib Python pre-pass)
    s3://dex-raw-landing-zone/polaris-warehouse/ucc_co/debtors_lance
        (filter DEBTOR_TYPE='Organization' AND STATE='CO';
         normalize ORG_NAME via _lib Python pre-pass)

Output:
    s3://dex-raw-landing-zone/polaris-warehouse/bridges/fmcsa_ucc_co_debtor_lance
...
Run:
    cd ~/hq-all/apps/data-engine-x && \\
      ...
      uv run --project . python3 scripts/build_bridge_fmcsa_ucc_co_debtor_lance.py --apply

Dry-run (no Lance write; counts + tier breakdown only):
    doppler run --project hq-all --config prd -- \\
      uv run --project . python3 scripts/build_bridge_fmcsa_ucc_co_debtor_lance.py --dry-run
"""
...
# CRITICAL (C2 / validator P1): the method-definition and
# method-version-definition helpers are INTENTIONALLY OMITTED.
# legal_name_state_exact_co v1.0.0 is REUSED from PR #615 (ppp_ucc_co_debtor).
...
# ppp/sam/usaspending CO UCC bridge consumers. Mirror: build_bridge_sam_ucc_co_debtor_lance.py.
...
BRIDGE_NAME = "fmcsa_ucc_co_debtor"           # naked (ops.bridges convention)
DATASET_SLUG = "fmcsa_ucc_co_debtor_lance"    # R2/lance suffix
METHOD_NAME = "legal_name_state_exact_co"     # REUSED -- registered by PR #615
METHOD_SEMVER = "1.0.0"                       # REUSED -- version row from PR #615
BRIDGE_VERSION = "1.0.0"

COLLISION_THRESHOLD = 50  # >50 fan-out on either side -> rejected (C5)
# Validator-calibrated 2026-05-21. ~54.4% of est_rows_matched=134,196.
# HARD FAIL if rows_matched < MIN_ROWS_MATCHED before any Lance write (C9).
MIN_ROWS_MATCHED = 73_000

SOURCE_LEFT = "fmcsa"
SOURCE_RIGHT = "ucc_co_debtor"
...
logger = logging.getLogger("build_bridge_fmcsa_ucc_co_debtor_lance")
...
def _materialize_inputs(storage_options: dict) -> tuple:
    """Load FMCSA CO carriers + UCC CO org debtors into Arrow tables.

    FMCSA left side (C8 -- CO filter, C3 -- Python normalizer):
      - Scanner filter: phy_state='CO' (validator-confirmed lowercase col name;
        value is the literal string 'CO'). 79,736 CO carriers total.
    ...
    UCC right side (C8 -- org filter + CO, C3 -- Python normalizer):
      - Scanner filter: DEBTOR_TYPE='Organization' (validator-confirmed live
        PyLance scan 2026-05-21: distinct set is exactly {'Organization',
        'Individual'}; 'Organization' literal is identical to the CA dataset)
        AND STATE='CO' (structural no-op -- every ucc_co row carries 'CO';
        kept for CA-template parity and explicitness per directive C8).
      - Project: UCC1_NUM, UCC3_NUM, DEBTOR_TYPE, ORG_NAME, ADDR1, CITY,
        STATE, POSTAL_CODE (filing-grain -- mirrors CA template fidelity).
    ...
    # FMCSA left side.
    logger.info("opening fmcsa/carrier_essentials_lance (phy_state='CO') ...")
    ...
    fmcsa_tbl = fmcsa_ds.scanner(
        ...
        filter=pc.field("phy_state") == "CO",
    ).to_table()
    rows_fmcsa_co = len(fmcsa_tbl)
    logger.info(
        "  fmcsa carrier_essentials_lance total=%d  CO carriers=%d",
        fmcsa_total, rows_fmcsa_co,
    )
    ...
    # UCC right side (filing-grain, mirrors CA template fidelity).
    logger.info(
        "opening ucc_co/debtors_lance "
        "(DEBTOR_TYPE='Organization', STATE='CO') ..."
    )
    ...
    rows_ucc_co_org = len(ucc_tbl)
    logger.info(
        "  ucc debtors_lance total=%d  CO org debtors=%d",
        ucc_total, rows_ucc_co_org,
    )
...
def _build_match_table(
    ...
    *,
    ...
    # 1. Inner JOIN on normalized legal name (both sides CO-pinned at scanner).
    con.execute(
        f"""
...
def main() -> int:
    """Build the FMCSA CO carriers x CO UCC-1 debtors Pattern B bridge."""
    parser = argparse.ArgumentParser(
        description=(
            "FMCSA CO carrier x CO UCC-1 debtor Pattern B bridge. "
            "Matches CO motor carriers to CO UCC-1 debtor filings on normalized "
            "legal_name -- identifies carriers carrying UCC liens (equipment finance, "
            "factoring). Method: legal_name_state_exact_co v1.0.0 (REUSE -- PR #615)."
        )
    )
    ...
    logger.info(
        "bridge: %s  method=%s v%s (REUSED -- PR #615)  normalizer=v%s  mode=%s",
    ...
    logger.info(
        "estimate: 134,196 rows_matched "
        "(platinum=9,024; gold=104,207; silver=20,965; rejected=38,220)"
    )
    ...
    # REUSER: only register_bridge (no method-definition helpers per C2).
    register_bridge(
        bridge_name=BRIDGE_NAME,
        ...
        description=(
            "FMCSA CO motor carriers x CO UCC-1 debtor filings (Organization-type) -- "
            "normalized legal_name exact match. Identifies which CO motor carriers "
            "carry UCC liens (equipment loans, asset-based lending, factoring). "
            "Method: legal_name_state_exact_co v1.0.0 (REUSE -- 4th consumer; "
            "defined by ppp_ucc_co_debtor PR #615). "
            "FMCSA side: phy_state='CO' CO carriers from carrier_essentials_lance. "
            "UCC side: DEBTOR_TYPE='Organization' AND STATE='CO' filing-grain from "
            "ucc_co/debtors_lance. Dual BTREE on carrier_name_normalized + "
