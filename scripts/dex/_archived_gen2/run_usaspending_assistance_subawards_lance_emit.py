"""s2 — USAspending assistance subawards Lance emit (Pattern A pull-through).

Reads r2://dex-raw-landing-zone/usaspending/assistance_subawards/year=2026/data.parquet
(one-shot CSV-bulk ingest 2026-05-09 via scripts/run_usaspending_subawards_ingest.py;
54,019 rows per validator probe 2026-05-16) via DuckDB-on-R2 (httpfs + R2 SECRET).
TRY_CASTs 21 trap VARCHARs at write-time (L49 — *_amount AS DOUBLE,
*_date AS DATE, *_fiscal_year AS INTEGER) so downstream consumers don't re-cast.
NOTE: assistance feed has 21 trap VARCHARs (22 contract - 1 contract-only date field)
per validator probe 2026-05-16.
Writes Lance to s3://dex-raw-landing-zone/polaris-warehouse/usaspending/
assistance_subawards_lance with BTREE x 4: prime_award_unique_key + subaward_number
+ subawardee_uei + prime_awardee_uei (all 4 validator-confirmed present).

Floor 48,617 (0.9 x 54,019 measured).
...
Run via (DETACH IS MANDATORY):
    cd ~/hq-all/apps/data-engine-x && \\
      ...
      modal run --detach scripts/run_usaspending_assistance_subawards_lance_emit.py::run
"""
...
app = modal.App("data-engine-x-usaspending-assistance-subawards-lance-emit")
...
DATASET_SLUG = "assistance_subawards_lance"
SRC_PARQUET_URI = (
    "r2://dex-raw-landing-zone/usaspending/assistance_subawards/"
    "year=2026/data.parquet"
)
LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/usaspending/assistance_subawards_lance"
MIN_ROW_FLOOR = 48_617
BTREE_COLUMNS = [
...
# L49 — 21 trap VARCHARs in assistance feed (validator probe 2026-05-16).
# Cast policy: *_amount and the IIJA/COVID supplementals → DOUBLE;
# *_date → DATE; *_fiscal_year (integer year stored as string) → INTEGER.
# NOTE: assistance feed has 21 trap VARCHARs (one fewer DATE field than contract feed).
# The missing field is contract-feed-only (potential end date field absent in grants).
TRY_CAST_DOUBLE = [
    "prime_award_amount",
...
TRY_CAST_DATE = [
    ...
    # NB: assistance grants do not carry the contract period potential end date.
    # This is the 1-field delta between contract (22) and assistance (21) feeds.
...
# Total = 12 DOUBLE + 6 DATE + 3 INTEGER = 21. Matches validator probe.
...
def _build_select_sql(parquet_uri: str) -> str:
    ...
    Assistance feed has 21 trap VARCHARs (contract-only field omitted):
