"""WA L&I Contractor Licenses -> Lance emit (Pattern A, one-off operator-fired).

Reads the Washington State Dept of Labor & Industries (L&I) "Contractor License
Data - General" registry (Socrata data.wa.gov / m8qx-ubtq) and writes a Lance
dataset at:
    s3://dex-raw-landing-zone/polaris-warehouse/licensure/wa_lni_contractors_lance

R2 Parquet intermediate at:
    s3://dex-raw-landing-zone/wa-lni-contractors/release={release}/data.parquet

Source: https://data.wa.gov/resource/m8qx-ubtq.csv (clean Socrata CSV; field-name
header, no title row). 160,954 rows confirmed 2026-05-29.

Columns:
  - 23 verbatim from CSV (all VARCHAR per L9 "emit verbatim"):
      ...
      businesstypecodedesc, specialtycode1, specialtycode1desc, specialtycode2,
      specialtycode2desc, ubi, primaryprincipalname, statuscode,
      contractorlicensestatus, contractorlicensesuspenddate
  - business_name_normalized (additive, via normalize_entity_name UDF per L34).

BTREE indices on contractorlicensenumber + business_name_normalized +
contractorlicensetypecodedesc (Construction/Electrical/Plumbing/Elevator) +
businesstypecodedesc (GENERAL vs SPECIALTY -- the GC filter). Mirrors
run_tx_tdlr_lance_emit.py / run_az_roc_lance_emit.py.
...
Run via:
    cd ~/hq-all/apps/data-engine-x && \\
      ...
      uv run python scripts/run_wa_lni_contractors_lance_emit.py \\
        --csv-path "/tmp/wa_lni_contractors.csv" --release 2026-05-29
"""
...
DATASET_SLUG = "wa_lni_contractors_lance"
LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/licensure/wa_lni_contractors_lance"
MIN_ROW_FLOOR = 150_000  # probe parsed 160,954 on 2026-05-29
...
def emit(csv_path: str, release: str) -> dict:
    ...
    local_parquet = f"/tmp/wa-lni-{release}.parquet"
    r2_key = f"wa-lni-contractors/release={release}/data.parquet"
    ...
    con.execute(
        f"""
        COPY (
            SELECT *, py_normalize_entity("businessname") AS business_name_normalized
    ...
    for col in ("contractorlicensenumber", "business_name_normalized",
                "contractorlicensetypecodedesc", "businesstypecodedesc"):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="WA L&I Contractor Licenses -> Lance emit (one-off)")
