"""s6 - Glassdoor /company-salaries Lance emit (Pattern A, Modal-hosted).

Reads r2://dex-raw-landing-zone/glassdoor/company_salaries/snapshot=YYYY-MM-DD/
data.parquet via DuckDB-on-R2. job_title_normalized already lands in source
(service does lowercase trim at ingest time), so this emit is mostly a pull-
through. Writes Lance to
s3://dex-raw-landing-zone/polaris-warehouse/glassdoor/company_salaries_lance.

2 BTREE: glassdoor_company_id, job_title_normalized.

(_normalize_domain_sql / _normalize_entity_sql helpers retained for parity
with sibling s6 scripts even though not used inline here — keeps every s6 emit
mechanically consistent for future maintainers.)

Run via (DETACH IS MANDATORY per L47):
    cd ~/hq-all/apps/data-engine-x && \\
      doppler run --project hq-all --config prd -- \\
      modal run --detach scripts/run_glassdoor_company_salaries_lance_emit.py::run
"""
...
app = modal.App("data-engine-x-glassdoor-company-salaries-lance-emit")
...
DATASET_SLUG = "glassdoor_company_salaries_lance"
LANCE_URI = (
    "s3://dex-raw-landing-zone/polaris-warehouse/glassdoor/company_salaries_lance"
)
MIN_ROW_FLOOR = 5  # ceil(0.9 × 5)
BTREE_COLUMNS = ["glassdoor_company_id", "job_title_normalized"]
...
# Retained for sibling-s6 parity (unused inline — salaries already lands with
# job_title_normalized + has no website/domain column).
...
def emit() -> dict:
    ...
    src_parquet_uri = (
        f"r2://dex-raw-landing-zone/glassdoor/company_salaries/"
