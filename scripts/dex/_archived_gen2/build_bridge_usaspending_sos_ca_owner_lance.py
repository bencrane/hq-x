"""s1 - USAspending CA recipients × CA SoS entities owner-identity bridge (Pattern B).

Pattern B exact-match bridge: USAspending federal contract recipients
(contracts_lance filtered recipient_state_code='CA' + DISTINCT (recipient_uei,
recipient_name) → normalized on-the-fly Python-side) × CA SoS entities
(entity_name_normalized produced by PR #464).

Spine pivot (validator probe #1): the metrics-only rollup dataset (134,153 rows)
has NO recipient_name or state — cannot anchor a name-based bridge. Correct LEFT
spine is usaspending.contracts_lance filtered recipient_state_code='CA' +
DISTINCT (recipient_uei, recipient_name) = 13,521 distinct CA recipients.
Downstream enrichment cycles JOIN this bridge's recipient_uei back to the UEI
aggregation dataset (99.9% UEI alignment per probe #1c: 134,013 / 134,153
intersection).
...
and CSLB × SoS bridges' provenance trail). This is the third REUSER
of this method (prior reusers: PR #466 UCC CA × CA SoS, PR #482 CSLB × CA SoS;
publisher: PR #464 SBA × CA SoS).
...
MIN_ROWS_MATCHED = 10_000 (validator-recalibrated 2026-05-17 post full-corpus
probe: 14,964 non-rejected matches — 8,465 platinum + 383 gold + 6,116 silver
+ 52 rejected. 10K floor catches catastrophic failure without false-tripping;
~33% headroom vs probe yield).

Inputs:
    s3://dex-raw-landing-zone/polaris-warehouse/usaspending/contracts_lance
        (recipient_uei + recipient_name + recipient_state_code; filter CA;
        DISTINCT (recipient_uei, recipient_name) → 13,521 rows; normalize
    ...
    s3://dex-raw-landing-zone/polaris-warehouse/sos/ca_entities_lance
        ...
        no jurisdiction filter — precedent PR #464/#466/#482 omits it; dataset
        name is CA-implicit; foreign LLCs registered in CA appear here and
        are included intentionally for downstream owner-identity chain)

Output:
    s3://dex-raw-landing-zone/polaris-warehouse/bridges/usaspending_sos_ca_owner_lance
    (BTREE on recipient_uei AND sos_entity_num; dual-BTREE per validator p6 /
    precedent PR #466/#482)

Normalizer (validator §"Normalize-on-the-fly"):
    USAspending side has NO pre-normalized name column. With only 13.5K LEFT
    rows post-CA-filter + DISTINCT, call normalize_entity_name(name) in a
    ...
    — the corpus is small enough to normalize in-process.
...
Match method REUSE (validator p2 / L21):
    ...
    NOT IMPORTED — see L21 / validator p5 — REUSER pattern mirrors PR #482.)

Tier rule (symmetric two-sided per PR #466/#482):
...
Plain Python (path-a) — NOT Modal:
    cd ~/hq-all/apps/data-engine-x && \\
      ...
      uv run python scripts/build_bridge_usaspending_sos_ca_owner_lance.py --apply

Dry-run (no Lance write, bridge run marked failed-dry-run):
    uv run python scripts/build_bridge_usaspending_sos_ca_owner_lance.py
"""
...
# PR #466 UCC CA × CA SoS, PR #482 CSLB × CA SoS).
...
# and CSLB × SoS bridges' provenance trail).
...
BRIDGE_NAME = "usaspending_sos_ca_owner"          # NAKED — no _lance suffix (ops.bridges convention)
DATASET_SLUG = "usaspending_sos_ca_owner_lance"   # _lance suffix for R2/Polaris/ops.data_sources
...
# Validator-recalibrated 2026-05-17 post full-corpus probe.
# contracts_lance CA-filtered + DISTINCT = 13,521 LEFT rows;
# probe yield: 14,964 non-rejected (8,465 platinum + 383 gold + 6,116 silver +
# 52 rejected). 10K floor catches catastrophic failure (normalizer regression,
# CA-filter regression) without false-tripping (~33% headroom vs probe).
...
BRIDGE_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/usaspending_sos_ca_owner_lance"
...
def _materialize_inputs(storage_options: dict) -> tuple:
    """Load USAspending CA recipients + CA SoS entities into Arrow tables.

    USAspending side (spine pivot per validator probe #1):
      - Read contracts_lance (the correct spine per validator probe #1 — has
        recipient_uei + recipient_name + recipient_state_code).
      - Push-down filter: recipient_state_code = 'CA' (probe #2: 854,402 rows
        → all USA, no Canada-CA collision; no pop_state fallback needed).
      - DuckDB DISTINCT (recipient_uei, recipient_name) → 13,521 canonical pairs.
    ...
    logger.info("opening usaspending/contracts_lance (CA filter + DISTINCT) ...")
    contracts_ds = lance.dataset(LEFT_LANCE_URI, storage_options=storage_options)

    # Push-down filter: recipient_state_code = 'CA' (validator probe #2).
    ca_filter = pc.field("recipient_state_code") == "CA"
    contracts_raw = contracts_ds.scanner(
        columns=["recipient_uei", "recipient_name", "recipient_state_code"],
        filter=ca_filter,
    ).to_table()
    logger.info("  contracts_lance CA rows (pre-DISTINCT): %d", len(contracts_raw))

    # DISTINCT (recipient_uei, recipient_name) in DuckDB to collapse 854K
    # transaction rows → ~13.5K canonical CA UEI+name pairs.
    ...
    logger.info("  contracts_lance CA distinct (recipient_uei, recipient_name): %d rows", rows_after_distinct)

    # Python-side normalize: attach recipient_name_normalized column.
    # NOT a DuckDB UDF — corpus is ~13.5K rows, cheap to normalize in-process.
...
def _build_match_table(
    ...
    *,
    ...
    Join key: recipient_name_normalized (USAspending) = entity_name_normalized (SoS).
    Name-only — state is implicit from CA-filter on LEFT + CA-SoS dataset name.
    Fan-out: separate usaspending_fan_out (per normalized name across USAspending rows)
    ...
    # 2. Fan-out counts (symmetric two-sided per PR #466/#482 pattern).
    #    usaspending_fan_out: # of USAspending rows sharing this normalized name.
    ...
    # 3. Tier rule (symmetric two-sided per PR #466/#482):
...
def _write_bridge_lance(con, storage_options: dict) -> int:
    """Write bridge_match to Lance via Arrow-bridge + dual BTREE (recipient_uei, sos_entity_num)."""
    import lance
    ...
    with lance_commit_lock(DATASET_SLUG):
        ...
        # Dual BTREE per contract §"Hard invariants" #7 / precedent PR #466/#482.
        try:
...
def main() -> int:
    """Build the USAspending CA recipients × CA SoS owner-identity bridge."""
    parser = argparse.ArgumentParser(
        description="USAspending CA recipients × CA SoS entities Pattern B fuzzy bridge generator."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    register_bridge(
        ...
        description=(
            "USAspending CA recipients x CA SoS entities — legal-name exact match. "
            "Reuses legal_name_state_exact_ca v1.0.0 method registered by PR #464 (SBA x SoS publisher). "
            "Third REUSER after PR #466 (UCC CA x CA SoS) and PR #482 (CSLB x CA SoS). "
            "Spine: usaspending.contracts_lance filtered recipient_state_code='CA' + "
            "DISTINCT (recipient_uei, recipient_name); 99.9% UEI alignment enables "
            "downstream enriched-cohort cycle via the UEI aggregation dataset."
        ),
    )
