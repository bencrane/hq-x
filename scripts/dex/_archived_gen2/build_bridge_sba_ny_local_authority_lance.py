"""SBA NY borrowers × NY Local Authority Procurements — Pattern B REUSER bridge.

Pattern B exact-match bridge: SBA NY borrowers (borrstate='NY') × NY Local
Authority procurements (vendor_state='NY') on normalized name.

Method: legal_name_state_exact_ny v1.0.0 (REUSED). FIFTH REUSER. Per L21,
register_match_method[_version] helpers INTENTIONALLY OMITTED.

Bridge version: 1.0.0
COLLISION_THRESHOLD = 50
MIN_ROWS_MATCHED = 500 (Local Authorities cover ~65K rows; NY-vendor subset
is the majority; expect a few hundred to ~thousand matches via name-state-exact).

Inputs:
    s3://dex-raw-landing-zone/polaris-warehouse/sba/borrowers_lance
        (borrstate='NY'; DISTINCT.)
    s3://dex-raw-landing-zone/polaris-warehouse/nystate/local_authority_procurements_lance
        (filter vendor_state='NY' AND vendor_name_normalized is_valid.)

Output:
    s3://dex-raw-landing-zone/polaris-warehouse/bridges/sba_ny_local_authority_lance
...
BRIDGE_NAME = "sba_ny_local_authority"
DATASET_SLUG = "sba_ny_local_authority_lance"
METHOD_NAME = "legal_name_state_exact_ny"
METHOD_SEMVER = "1.0.0"
BRIDGE_VERSION = "1.0.0"

COLLISION_THRESHOLD = 50
MIN_ROWS_MATCHED = 500

SOURCE_LEFT = "sba_borrowers_lance"
SOURCE_RIGHT = "ny_local_authority_procurements_lance"

LEFT_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/sba/borrowers_lance"
RIGHT_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/nystate/local_authority_procurements_lance"
BRIDGE_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/sba_ny_local_authority_lance"
...
def _materialize_inputs(storage_options: dict) -> tuple:
    ...
    logger.info("opening nystate/local_authority_procurements_lance (vendor_state=NY) ...")
    la_ds = lance.dataset(RIGHT_LANCE_URI, storage_options=storage_options)
    la_filter = (
        (pc.field("vendor_state") == "NY")
        & pc.field("vendor_name_normalized").is_valid()
    )
    la_tbl = la_ds.scanner(
        columns=[
    ...
    logger.info("  local_authority_procurements_lance (vendor_state=NY): %d rows", len(la_tbl))

    return sba_branded_arrow, la_tbl, rows_sba_distinct, len(la_tbl)


def _build_match_table(sba_tbl, la_tbl, *, bridge_run_id: str, generated_at_iso: str) -> tuple:
    import duckdb
    ...
    con.register("la", la_tbl)
    logger.info("  registered: sba=%d  la=%d",
                con.execute("SELECT COUNT(*) FROM sba").fetchone()[0],
                con.execute("SELECT COUNT(*) FROM la").fetchone()[0])
    ...
    con.execute(
        ...
        SELECT
            ...
            la.contract_id,
            la.authority_name                        AS la_authority_name,
            la.fiscal_year_end_date                  AS la_fiscal_year_end_date,
            la.procurement_description               AS la_procurement_description,
            la.type_of_procurement                   AS la_type_of_procurement,
            la.award_process                         AS la_award_process,
            la.award_date                            AS la_award_date,
            la.end_date                              AS la_end_date,
            la.contract_amount                       AS la_contract_amount,
        ...
        FROM sba
        JOIN la
          ON sba.sba_legal_name_normalized = la.vendor_name_normalized
        """
    )
    ...
    con.execute(
        """
        CREATE TEMP TABLE la_fanout AS
        SELECT sba_legal_name_normalized,
               COUNT(DISTINCT contract_id) AS la_fan_out
        FROM bridge_raw GROUP BY 1
        """
    )
        ...
        SELECT
            ...
            CASE
                WHEN sf.sba_fan_out > {COLLISION_THRESHOLD}
                  OR lf.la_fan_out > {COLLISION_THRESHOLD}
                    THEN 'rejected'
                WHEN sf.sba_fan_out = 1 AND lf.la_fan_out = 1
                    THEN 'platinum'
                WHEN sf.sba_fan_out = 1 OR  lf.la_fan_out = 1
                    THEN 'gold'
                ELSE 'silver'
            END AS confidence_tier
        FROM bridge_raw b
        JOIN sba_fanout sf USING (sba_legal_name_normalized)
        JOIN la_fanout  lf USING (sba_legal_name_normalized)
        """
    )
...
def main() -> int:
    parser = argparse.ArgumentParser(description="SBA NY × NY Local Authority Procurements Pattern B REUSER bridge.")
    ...
    register_bridge(
        ...
        description=(
            "SBA NY borrowers x NY Local Authority Procurements (vendor_state='NY') — "
            "legal-name exact match. Reuses legal_name_state_exact_ny v1.0.0 "
            "(publisher PR #513). FIFTH REUSER. Dual BTREE on sba_legal_name_normalized "
            "+ contract_id (synthetic sha1[:16] derived from authority+vendor+date+amount)."
        ),
    )
    ...
    try:
        sba_tbl, la_tbl, rows_left, rows_right = _materialize_inputs(storage_options)
        con, counts = _build_match_table(
            sba_tbl, la_tbl, bridge_run_id=bridge_run_id, generated_at_iso=started_at.isoformat(),
        )
