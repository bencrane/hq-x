#!/usr/bin/env python3
"""Cycle 3: SBIC × CA UCC secured parties via name-stem prefix (Pattern B Lance).

Identifies which SBIC fund-manager firms appear as secured parties on
California UCC-1 filings — i.e. SBICs with active CA-collateral lending
positions. Catches both:
  - direct: SBIC manager firm IS the secured party (Apollo Investment Corp
    appears verbatim on a CA UCC-1)
  - sibling: SBIC and UCC entity share a brand-stem (Live Oak SBIC LLC ↔
    Live Oak Banking Company)

UCC filing state is CA (collateral is CA-domiciled), but the secured party
(lender) can be anywhere. The match is name-only, not state-filtered — the
cohort of interest is ALL SBICs with active CA lending, not just CA-HQ'd.

Pattern B per inventory/DATA-FACTORY-ARCHITECTURE-PATTERNS.md:
  - inputs: sba.sbic_directory_lance + ucc_ca.secured_parties_lance
  - method: company_name_stem_prefix v1.0.0 (REUSED from Cycle 2 SBIC × SBA
    7(a) lender bridge per L21 — do NOT re-register method/version)
  - normalizer: _lib/entity_name_normalize.py v1.0.0 on both sides
  - UCC filter: SECURED_PARTY_TYPE = 'Organization' (≈98% of rows)
  - UCC dedupe: GROUP BY normalized ORG_NAME, aggregate filing counts +
    state/city distinct sets
  - tiers: same as Cycle 2

Floor: 1 (exploratory; some SBIC managers will appear, some won't).

Usage:
  cd apps/data-engine-x && doppler run --project hq-all --config prd -- \\
      uv run python scripts/build_bridge_sbic_ucc_ca_secured_party_lance.py --apply
"""
...
logger = logging.getLogger("build_bridge_sbic_ucc_ca_secured_party_lance")

# Bridge identity ------------------------------------------------------------
BRIDGE_NAME = "sbic_ucc_ca_secured_party_lance"
METHOD_NAME = "company_name_stem_prefix"  # reused from Cycle 2 (L21)
METHOD_SEMVER = "1.0.0"
BRIDGE_VERSION = "1.0.0"

SOURCE_LEFT = "sba_sbic_directory_lance"
SOURCE_RIGHT = "ucc_ca_secured_parties_lance"
...
BRIDGE_LANCE_URI = (
    ...
    "sbic_ucc_ca_secured_party_lance"
)
DATASET_SLUG = "sbic_ucc_ca_secured_party_lance"
...
def _materialize_inputs(storage_options: dict) -> tuple:
    ...
    ucc_ds = lance.dataset(UCC_CA_LANCE_URI, storage_options=storage_options)
    ...
    logger.info("  ucc_ca organization secured-party rows: %d", rows_ucc)
...
def _build_match_table(
    ...
    *,
    ...
    logger.info("aggregating ucc_ca secured parties by normalized ORG_NAME ...")
    con.execute(
        """
...
def _ensure_registry() -> None:
    ...
    by Cycle 2 (SBIC × SBA 7(a) lender). Per L21: reuse shared method, do NOT
    call register_match_method_version (input_columns_left/right captured at
    first registration; overwriting would clobber Cycle 2's provenance).
    """
    register_bridge(
        ...
        description=(
            "SBA SBIC Directory × CA UCC secured parties by name-stem "
            "prefix. Identifies SBIC fund-managers active as secured "
            "parties on California UCC-1 filings (CA-collateral lending "
...
def main() -> int:
    ...
    logger.info("inputs: %s + %s", SBIC_LANCE_URI, UCC_CA_LANCE_URI)
