#!/usr/bin/env python3
"""Emit usaspending/awards_by_naics_month_lance — ALL-years naics×month rollup.
...
(naics_code, action_month, kind) rollup.

Schema (one row per (naics_code, action_month, kind) tuple):
  naics_code      VARCHAR  — 2-6 digit NAICS code
...
BTREE on: naics_code, action_month.

MIN_ROW_FLOOR: 200,000 (validator-stamped; ~250K rows expected for all years).

Usage:
  doppler run --project hq-all --config prd -- \\
    uv run python apps/data-engine-x/scripts/emit_usaspending_awards_by_naics_month_lance.py --apply

  doppler run --project hq-all --config prd -- \\
    uv run python apps/data-engine-x/scripts/emit_usaspending_awards_by_naics_month_lance.py --dry-run
"""
...
logger = logging.getLogger("emit_usaspending_awards_by_naics_month_lance")

DATASET_SLUG = "usaspending_awards_by_naics_month_lance"
OUT_LANCE_URI = (
    "s3://dex-raw-landing-zone/polaris-warehouse/usaspending/awards_by_naics_month_lance"
)
AWARDS_LANCE_URI = (
...
BTREE_COLS = ["naics_code", "action_month"]
