#!/usr/bin/env python3
"""s5 — Emit UCC CA secured parties as a Lance dataset on R2.

Reads: s3://dex-raw-landing-zone/ucc-ca/master/snapshot=2026-05-01/parsed/secured_parties.parquet
Writes: s3://dex-raw-landing-zone/polaris-warehouse/ucc_ca/secured_parties_lance/

BTREE index on UCC1_NUM (join key back to filings).

Usage:
    doppler run --project hq-all --config prd -- \\
        uv run --quiet python3 apps/data-engine-x/scripts/emit_ucc_ca_secured_parties_lance.py \\
            --apply
"""
...
PARQUET_URI = (
    "r2://dex-raw-landing-zone/ucc-ca/master/snapshot=2026-05-01/parsed/secured_parties.parquet"
)
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(description="Emit UCC CA secured parties Lance (s5)")
    ...
    LOG.info("s5 complete: %d rows, %.1fs", lance_count, time.time() - t0)
    return 0
