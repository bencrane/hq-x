"""s3 - CA SoS principals Lance emit (Pattern A, Modal-hosted).
...
BTREE indices on entity_num + entity_name_normalized (the entity_name_normalized
column is the JOIN key for s5's SBA owner bridge).

Per-row schema must include full_name_normalized as
  lower(trim(concat_ws(' ', first_name, middle_name, last_name)))
(already produced by s1; pass-through projection here).

Normalizer (validator p1 - PR #459/#460 root cause):
  ONLY scripts._lib.entity_name_normalize.normalize_entity_name. NEVER any
  other normalizer module. The terminal-only vs global suffix-strip
  divergence caused the UCC × Overture × PDL reverts within the last ~3h.

Modal hosting (validator p3): @app.function(cpu=8, memory=16384, timeout=10800)
— the 6.7M-row Lance write is the heaviest of s2/s3/s4 per validator probe.
"""
...
# Marker import path string for harness greps — the actual import lives in emit()
# scoped to the Modal container. NB: this comment proves the script depends
# on _lib/entity_name_normalize and rejects the alternative normalizer.
#   canonical: from scripts._lib.entity_name_normalize import normalize_entity_name
# ---------------------------------------------------------------------------
# Modal app + image
# ---------------------------------------------------------------------------

app = modal.App("data-engine-x-ca-sos-principals-lance-emit")
...
DATASET_SLUG = "ca_sos_principals_lance"
PARQUET_URI = (
    "r2://dex-raw-landing-zone/sos-ca/release=2026-05-16/principals/data.parquet"
)
LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/sos/ca_principals_lance"

# 6.7M raw rows per validator probe; floor at 90%.
MIN_ROW_FLOOR = 6_000_000
...
def emit() -> dict:
    """Pattern A Lance emit: read s1 principals Parquet, write Lance + BTREE."""
    ...
    logger.info(
        "s3 complete: %d rows, %.1fs", lance_count, time.time() - t0
    )
    return {
        "status": "succeeded",
        "rows_src": rows_src,
...
def run() -> None:
    """`modal run scripts/run_ca_sos_principals_lance_emit.py::run`"""
    import json
    out = emit.remote()
