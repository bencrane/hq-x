"""SEC Form ADV Part 1 — base_a Lance emit (Pattern A thin wrapper).

Reads ZSTD Parquet at:
    r2://dex-raw-landing-zone/sec-adv/year=2024/table=base_a/*.parquet
...
Source row count: 471,129 (2024 snapshot).
Floor (99%): 466,418.

Invocation:
    cd ~/hq-all/apps/data-engine-x && \\
      doppler run --project hq-all --config prd -- \\
      python3 scripts/run_sec_adv_base_a_lance_emit.py --apply
"""
...
CONFIG = LanceEmitConfig(
    dataset_slug="sec_adv_base_a_lance",
    r2_bucket="dex-raw-landing-zone",
    parquet_input_prefix="sec-adv",
    parquet_file_pattern="table=base_a/*.parquet",
    partition_mode="multi_year",
    multi_year_filter=[2024],
    lance_uri="s3://dex-raw-landing-zone/polaris-warehouse/sec_adv/base_a_lance",
