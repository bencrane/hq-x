#!/usr/bin/env python3
"""Bridge generator: CO SoS Good Standing entity × Overture US Place — address-keyed.

Sibling of sos_ca_overture_address / sos_fl_overture_address / sos_ny_overture_address.

Inputs:
  CO SoS:   `polaris-warehouse/sos/co_entities_lance`              (~3.05M rows)
  Overture: `polaris-warehouse/overture/us_places_lance`           (~15.95M rows)

Filter on CO SoS side: `entitystatus = 'Good Standing'` (~1.04M of 3.05M).
Address: `principaladdress1` + `principaladdress2` (multi-line) with mailing fallback.

Join key (composite, exact-equality after normalization):
  (address_base_normalized, zip5, 2-letter_state)

Fan-out tiering (same convention as siblings):
  platinum = 1:1
  gold     = 1:N or N:1
  silver   = N:M (fan-out ≤ 50 on each side)
  rejected = any fan-out > 50

Output: `polaris-warehouse/bridges/sos_co_overture_address_lance/`
Audit:  ops.bridge_generation_runs (bridge_name='sos_co_overture_address')
Method: REUSES `address_base_state_zip_exact` v1.0.0.
Floor:  ≥ 30,000 matched rows (CO is smaller than CA/FL/NY; conservative floor).
"""
...
logger = logging.getLogger("build_bridge_sos_co_overture_address_lance")

BRIDGE_NAME = "sos_co_overture_address"
METHOD_NAME = "address_base_state_zip_exact"
METHOD_SEMVER = "1.0.0"
BRIDGE_VERSION = "1.0.0"

SOURCE_LEFT = "sos_co_entities_lance"
SOURCE_RIGHT = "overture_us_places_lance"

SOS_CO_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/sos/co_entities_lance"
OVERTURE_PLACES_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/overture/us_places_lance"
BRIDGE_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/sos_co_overture_address_lance"
DATASET_SLUG = "sos_co_overture_address_lance"
...
def _materialize_inputs(storage_options: dict) -> tuple:
    """Load CO SoS Good Standing entities + Overture US Places; normalize; pre-dedup."""
    import lance
    import pyarrow as pa
    import pyarrow.compute as pc

    logger.info("opening sos/co_entities_lance ...")
    sos_ds = lance.dataset(SOS_CO_LANCE_URI, storage_options=storage_options)
    sos_filter = (
        pc.field("entityid").is_valid()
        & pc.equal(pc.field("entitystatus"), "Good Standing")
    )
    sos_cols = [
        "entityid", "entityname", "entity_name_normalized",
        "entitystatus", "entitytype", "jurisdictonofformation", "entityformdate",
    ...
    logger.info("  sos/co_entities_lance (Good Standing): %d rows", rows_sos_raw)

    entity_id = sos_raw.column("entityid").to_pylist()
    entity_name = sos_raw.column("entityname").to_pylist()
    entity_name_norm = sos_raw.column("entity_name_normalized").to_pylist()
    status_v = sos_raw.column("entitystatus").to_pylist()
    entity_type = sos_raw.column("entitytype").to_pylist()
    jurisdiction = sos_raw.column("jurisdictonofformation").to_pylist()
    form_date = sos_raw.column("entityformdate").to_pylist()
    a1 = sos_raw.column("principaladdress1").to_pylist()
    a2 = sos_raw.column("principaladdress2").to_pylist()
    city_p = sos_raw.column("principalcity").to_pylist()
    state_p = sos_raw.column("principalstate").to_pylist()
    zip_p = sos_raw.column("principalzipcode").to_pylist()
    m1 = sos_raw.column("mailingaddress1").to_pylist()
    m2 = sos_raw.column("mailingaddress2").to_pylist()
    city_m = sos_raw.column("mailingcity").to_pylist()
    state_m = sos_raw.column("mailingstate").to_pylist()
    zip_m = sos_raw.column("mailingzipcode").to_pylist()
    ...
    for i in range(rows_sos_raw):
        ...
        key = (entity_id[i], state_raw, z, base)
        if key in seen:
            continue
        seen.add(key)
        sos_rows.append(
            {
                "sos_entity_id": entity_id[i],
    ...
    del sos_raw, entity_id, entity_name, entity_name_norm, status_v, entity_type
    del jurisdiction, form_date, a1, a2, city_p, state_p, zip_p
    del m1, m2, city_m, state_m, zip_m, seen

    logger.info(
        "  sos normalized + deduped: %d distinct (entity_id, state, zip5, base_address)",
        len(sos_rows),
    )
...
def _build_match_table(sos_arrow, overture_arrow, *, bridge_run_id: str, generated_at_iso: str):
    ...
    con.execute(
        ...
        SELECT sos_entity_id, COUNT(*) AS sos_fan_out
        FROM matched GROUP BY sos_entity_id
        """
    )
    con.execute(
        """
    ...
    con.execute(
        ...
        FROM matched m
        JOIN sos_fanout sf ON sf.sos_entity_id = m.sos_entity_id
...
def _write_bridge_lance(con, storage_options: dict) -> int:
    ...
    with lance_commit_lock(DATASET_SLUG):
        ...
        for col in ("sos_entity_id", "place_id", "address_base_normalized"):
            try:
...
def _ensure_registry() -> None:
    register_match_method(
        ...
        description=(
            ...
            "sides. CO SoS uses principaladdress1+principaladdress2 (multi-line) "
            "with mailingaddress1+mailingaddress2 fallback."
        ),
    )
    register_match_method_version(
        method_name=METHOD_NAME,
        ...
        input_columns_left=[
            "principaladdress1", "principaladdress2", "principalzipcode", "principalstate",
            "mailingaddress1", "mailingaddress2", "mailingzipcode", "mailingstate",
        ],
        input_columns_right=[
    ...
    register_bridge(
        ...
        description=(
            "CO Secretary of State Good Standing entities × Overture US Places — "
            "address-keyed exact match. Anchors on principaladdress1+principaladdress2 "
            "(with mailing fallback). Sibling of sos_ca/fl/ny_overture_address."
        ),
    )
...
def main() -> int:
    ...
    logger.info("inputs: sos/co_entities_lance + overture/us_places_lance (Arrow-bridge)")
