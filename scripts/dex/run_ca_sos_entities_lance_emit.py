"""s2 - CA SoS entities Lance emit (Pattern A, Modal-hosted).
...
downstream Pattern B bridges).

Pattern A discipline:
  - wrap lance.write_dataset in lance_commit_lock("ca_sos_entities_lance")
  - compact_files() + cleanup_old_versions(timedelta(days=7))
  - mode="overwrite" (s1 output is a single release-stamped Parquet)

Active-CA-entity filter columns projected per validator non-blocking #6:
  entity_status, standing_sos, suspension_date, last_si_file_date.

Modal hosting (validator p3): @app.function(cpu=8, memory=16384, timeout=7200).
Run via:
    cd ~/hq-all/apps/data-engine-x && \\
      doppler run --project hq-all --config prd -- \\
      modal run scripts/run_ca_sos_entities_lance_emit.py::run
"""
...
app = modal.App("data-engine-x-ca-sos-entities-lance-emit")
...
DATASET_SLUG = "ca_sos_entities_lance"
PARQUET_URI = "r2://dex-raw-landing-zone/sos-ca/release=2026-05-16/entities/data.parquet"
LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/sos/ca_entities_lance"

# Validator p3: 4.5M+ row Lance write needs spill headroom.
MIN_ROW_FLOOR = 4_500_000
...
def emit() -> dict:
    ...
    with lance_commit_lock(DATASET_SLUG):
        # Project: all entities columns (s1 already typed; pass-through)
        # but force the entity-status family to be present + named per spec.
        reader = con.execute(
            f"""
            SELECT
                entity_num,
    ...
    logger.info(
        "s2 complete: %d rows, %.1fs", lance_count, time.time() - t0
    )
    return {
        "status": "succeeded",
        "rows_src": rows_src,
