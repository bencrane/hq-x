#!/usr/bin/env python3
"""Bridge generator: SBA × Overture × USPTO — three-axis identity triangulation.

Mirror of `ppp_overture_uspto_lance` for the SBA 7(a)+504 universe. Composes
two confirmed-attribute bridges that converge on the same SBA-borrower identity:

  base entity (left key): SBA borrower (legal_name_normalized, borrstate, borrzip)
  + attribute A:          bridges/sba_overture_address_lance  (storefront identity)
  + attribute B:          bridges/sba_uspto_owner_lance        (registered brand identity)

Every output row is a (SBA borrower ↔ Overture place_id ↔ USPTO serial_no)
triple where the same SBA-borrower paperwork-LLC is linked to both a physical
storefront (Overture) AND a registered trademark (USPTO). Cross-validated
identity — paperwork-LLC ↔ storefront ↔ registered brand.

Method: borrower_attribute_composite v1.0.0 (shared with PPP triple-axis;
inner-join across two attribute bridges sharing the same base entity,
worst-of tier composition).

Tier carried per axis:
  - overture_attach_tier   (from sba_overture_address.confidence_tier)
  - uspto_attach_tier      (from sba_uspto_owner.confidence_tier)
  - triple_axis_tier       (worst-of overture_attach_tier and uspto_attach_tier)

Expected volume: ~2.9M rows (73,541 SBA borrowers in both bridges × ~6.0 Overture
× ~7.0 USPTO avg fan-out).

The SBA cross-bridge cohort is smaller than the PPP one (73K vs 241K borrowers)
because sba_overture_address coverage is currently bounded by the
borrstreet_normalized side-scan (baked into sba/borrowers_lance v1.1.0 on
2026-05-27; ~18% of borrowers have populated street normalization today).
Subsequent emits of that source will lift this cohort materially.

Floor: ≥ 500,000 rows.

Output: `polaris-warehouse/bridges/sba_overture_uspto_lance/`
Audit:  ops.bridge_generation_runs (bridge_name='sba_overture_uspto')

Usage:
  doppler run --project hq-all --config prd -- \\
    uv run --with duckdb --with pylance --with pyarrow --with "psycopg[binary]" python \\
    apps/data-engine-x/scripts/build_bridge_sba_overture_uspto_lance.py --apply

  doppler run --project hq-all --config prd -- \\
    uv run --with duckdb --with pylance --with pyarrow --with "psycopg[binary]" python \\
    apps/data-engine-x/scripts/build_bridge_sba_overture_uspto_lance.py --dry-run
"""
...
logger = logging.getLogger("build_bridge_sba_overture_uspto_lance")

BRIDGE_NAME = "sba_overture_uspto"
METHOD_NAME = "borrower_attribute_composite"
METHOD_SEMVER = "1.0.0"
BRIDGE_VERSION = "1.0.0"

SOURCE_LEFT = "sba_overture_address_lance"
SOURCE_RIGHT = "sba_uspto_owner_lance"

PATH_A_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/sba_overture_address_lance"
PATH_B_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/sba_uspto_owner_lance"
BRIDGE_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/sba_overture_uspto_lance"
DATASET_SLUG = "sba_overture_uspto_lance"
...
TMP_DIR = f"/tmp/lance/sba_overture_uspto_{os.getpid()}"
...
def _materialize_inputs(storage_options: dict) -> tuple:
    ...
    logger.info("opening path A (SBA × Overture address): %s ...", PATH_A_URI)
    over_ds = lance.dataset(PATH_A_URI, storage_options=storage_options)
    # NOTE: sba_franchise_brands_set/naics_codes_set/lender_set are list<string>
    # on the SBA component bridges (vs comma-separated strings on PPP). Lance
    # 1.5.x has a known definition-buffer encoding cap on LIST columns that
    # surfaces as a struct-array-length mismatch on read (CLAUDE.md §L54).
    # Dropping them from the triple-axis output for v1; consumers can join
    # back to sba/borrowers_lance via (legal_name, borrstate, borrzip) if
    # franchise/NAICS rollups are needed.
    over_cols = [
        ...
        "sba_max_approval_date",
        "sba_min_approval_date",
        "sba_latest_loanstatus",
        "sba_has_pending_commit",
        "place_id",
    ...
    logger.info("  sba_overture_address_lance: %d rows", rows_over)

    logger.info("opening path B (SBA × USPTO owner): %s ...", PATH_B_URI)
    ...
    logger.info("  sba_uspto_owner_lance: %d rows", rows_uspto)
...
def _build_match_table(
    ...
    *,
    ...
    """INNER JOIN both attribute bridges on SBA borrower identity + tier composition."""
    import duckdb
    ...
    # Defensive: UPPER(TRIM()) both sides of borrstate during the join in case
    # of shape drift between the two component bridges' borrstate columns.
    # Verified consistent at build time (both uppercase 2-letter), but the
    # cost of the cast is negligible and the safety is worth keeping.
    logger.info(
        "INNER JOIN sba_overture_address × sba_uspto_owner on SBA borrower ..."
    )
    con.execute(
        f"""
        CREATE TEMP TABLE bridge_match AS
        SELECT
            -- SBA identity (the convergent base entity)
            o.sba_legal_name_normalized,
            o.sba_borrstate,
            o.sba_borrzip,
            o.sba_borrname_sample,
            o.sba_total_loans,
            o.sba_total_gross_approval,
            o.sba_max_approval_date,
            o.sba_min_approval_date,
            o.sba_latest_loanstatus,
            o.sba_has_pending_commit,
        ...
        FROM over o
        JOIN uspto u
          ON  u.sba_legal_name_normalized = o.sba_legal_name_normalized
          AND UPPER(TRIM(u.sba_borrstate)) = UPPER(TRIM(o.sba_borrstate))
          AND u.sba_borrzip                = o.sba_borrzip
        """
    )
    ...
    row_counts = con.execute(
        ...
        SELECT
          ...
          COUNT(DISTINCT (sba_legal_name_normalized, sba_borrstate, sba_borrzip)),
...
def _ensure_registry() -> None:
    # Method `borrower_attribute_composite` v1.0.0 already registered by the
    # PPP triple-axis build. register_match_method() is idempotent on name.
    register_match_method(
        ...
        description=(
            ...
            "(e.g. SBA/PPP borrower) has two confirmed-attribute attachments "
    ...
    register_match_method_version(
        ...
        output_value_description=(
            "(sba_borrower_identity, place_id, uspto_serial_no) triple — "
    ...
    register_bridge(
        ...
        description=(
            "SBA 7(a)+504 × Overture × USPTO triple-axis identity triangulation. "
            "Inner-joins sba_overture_address (storefront attribute) and "
            "sba_uspto_owner (registered-brand attribute) on the same SBA "
            "borrower identity (legal_name_normalized + borrstate + borrzip). "
            "Each output row is a cross-validated (paperwork-LLC ↔ storefront "
            "↔ trademark) identity chain — the gold standard for SBA capital-"
            "deployment cohort attribution, especially for the approved-but-"
            "not-yet-disbursed pipeline where the operator wants outreach-"
            "actionable identity per loan."
        ),
    )
...
def main() -> int:
    ...
    logger.info(
        "inputs: sba_overture_address_lance ⨝ sba_uspto_owner_lance on SBA borrower"
    )
    ...
    try:
        ...
        logger.info("  distinct SBA borrowers:   %d", counts["distinct_borrowers"])
