"""s4 - JSearch jobs Lance emit (Pattern A, Modal-hosted).

Reads r2://dex-raw-landing-zone/jsearch/snapshot=YYYY-MM-DD/jobs.parquet
via DuckDB-on-R2. Computes employer_name_normalized (inline _normalize_entity_sql,
byte-for-byte identical to emit_pdl_free_companies_lance.py:92-124 for join
parity with PDL legal_name_normalized) + employer_domain_normalized (inline
_normalize_domain_sql per build_bridge_sam_pdl_domain_lance.py:109-119
precedent). Writes Lance to
s3://dex-raw-landing-zone/polaris-warehouse/jsearch/jobs_lance.

6 BTREE: job_id, employer_name_normalized, employer_domain_normalized,
job_country, job_state, job_publisher.

Per-publisher duplicates preserved faithfully (source ingest invariant -
raw stays raw; canonicalization is downstream in s6).
...
Run via (DETACH IS MANDATORY per L47):
    cd ~/hq-all/apps/data-engine-x && \\
      ...
      modal run --detach scripts/run_jsearch_jobs_lance_emit.py::run
"""
...
app = modal.App("data-engine-x-jsearch-jobs-lance-emit")
...
DATASET_SLUG = "jsearch_jobs_lance"
LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/jsearch/jobs_lance"
MIN_ROW_FLOOR = 1_161
BTREE_COLUMNS = [
    "job_id", "employer_name_normalized", "employer_domain_normalized",
    "job_country", "job_state", "job_publisher",
]
...
# Inline domain normalization - IDENTICAL to build_bridge_sam_pdl_domain_lance.py:109-119.
...
# Inline entity-name normalization - IDENTICAL to emit_pdl_free_companies_lance.py:92-124.
# REQUIRED PARITY: pdl.legal_name_normalized was computed via this exact function;
# s5 name-fallback join compares jsearch.employer_name_normalized =
# pdl.legal_name_normalized, so both sides MUST use the same normalization.
...
def _build_select_sql(parquet_uri: str) -> str:
    employer_domain_expr = _normalize_domain_sql("employer_website")
    employer_name_norm_expr = _normalize_entity_sql("employer_name")
    return f"""
        SELECT
            * EXCLUDE (employer_name, employer_website),
            employer_name,
            employer_website,
            {employer_name_norm_expr} AS employer_name_normalized,
            CASE
              WHEN employer_website IS NULL OR employer_website = ''
                THEN NULL
              ELSE {employer_domain_expr}
            END AS employer_domain_normalized
...
def emit() -> dict:
    ...
    src_parquet_uri = (
        f"r2://dex-raw-landing-zone/jsearch/snapshot={snapshot_date}/jobs.parquet"
    )
