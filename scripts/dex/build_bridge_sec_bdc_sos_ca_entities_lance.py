"""SEC BDC Schedule-of-Investments portfolio companies × CA Secretary-of-State entities
...
portfolio_company_dba; entity_type='company') × CA SoS entities
(sos/ca_entities_lance: entity_name_normalized, entity_num).

NAME-ONLY match — the BDC side has no state, ZIP, domain, or officer
field. Match nationally against the full 9.39M-row CA SoS corpus.
...
This is the FIRST REUSER of company_name_exact_nostate v1.0.0
...
CA SoS right side:
  - Full sos/ca_entities_lance, 9,389,284 entity_name_normalized-valid rows.
  - entity_name_normalized is ALREADY _lib-normalized (PR #464).
  - JOIN DIRECTLY — do NOT re-normalize (prediction p3).
  - Carry entity_num + entity_status on every output row.
  - No state filter — NAME-ONLY, national match against full CA SoS corpus.

COLLISION_THRESHOLD = 50.
MIN_ROWS_MATCHED = 4_300 (validator-calibrated 2026-05-21; probe: 6,153
non-rejected = 2,331 platinum + 3,822 gold + 0 silver + 0 rejected).
Floor derivation: 4,300 / 6,153 = 69.9% — the ~70% convention per
...
Smoke target (validator-confirmed 2026-05-21):
  ...
  '6sense insights' → CA SoS entity_num='3566305'
  (entity_name='6SENSE INSIGHTS, INC.', entity_status='Active', platinum tier).
...
Match method REUSE (validator p1 / L21):
    ...
    NOT IMPORTED — see L21 / validator p1 — REUSER pattern.)

Bridge version: 1.0.0
Run:
    cd ~/hq-all/apps/data-engine-x && \\
      doppler run --project hq-all --config prd -- \\
      uv run --project . python3 scripts/build_bridge_sec_bdc_sos_ca_entities_lance.py --apply
...
# CRITICAL validator p1 / L21: the method-definition and method-version-definition
...
BRIDGE_NAME = "sec_bdc_sos_ca_entities"          # NAKED — no _lance suffix (ops.bridges convention)
DATASET_SLUG = "sec_bdc_sos_ca_entities_lance"   # _lance suffix for R2/Polaris/ops.data_sources
...
# BDC distinct normalized names: 13,459; probe: 6,153 non-rejected matched rows
# = 2,331 platinum + 3,822 gold + 0 silver + 0 rejected.
# Floor = 4,300 (~69.9% of probe; catches catastrophic failure from
# schema/normalizer regression without false-tripping on clean recipe runs).
MIN_ROWS_MATCHED = 4_300
...
BRIDGE_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/sec_bdc_sos_ca_entities_lance"

TMP_DIR = "/tmp/lance"

# ---------------------------------------------------------------------------
# Left-side hygiene — sector-header blacklist (verbatim from PR #627)
# Validator prediction p2: copy SECTOR_HEADER_BLACKLIST, _is_sector_header,
# and _build_bdc_name_set EXACTLY from build_bridge_sec_bdc_ppp_lance.py.
...
def _is_sector_header(raw_clean: str | None) -> bool:
    ...
    Two rules (validator prediction p2 / p3 in BDC×PPP):
...
SMOKE_TARGET_RAW = "6Sense Insights, Inc."
SMOKE_TARGET_EXPECTED_ENTITY_NUM = "3566305"
...
def _build_bdc_name_set(storage_options: dict) -> tuple[set[str], int]:
    ...
    Copied VERBATIM from build_bridge_sec_bdc_ppp_lance.py (PR #627) per
...
def _materialize_inputs(storage_options: dict) -> tuple:
    """Load BDC DISTINCT name set + CA SoS entities into Arrow tables.
    ...
    CA SoS side:
      - Full ca_entities_lance (no state filter — NAME-ONLY, national).
      - Filter: entity_name_normalized is_valid().
      - Project: entity_num, entity_name_normalized, entity_status.
      - DO NOT call normalize_entity_name on entity_name_normalized — it is
        already _lib-normalized (PR #464; validator prediction p3).
    """
    import lance
    ...
    logger.info("opening sos/ca_entities_lance (entity_name_normalized is_valid) ...")
    ...
    logger.info(
        "  sos ca_entities_lance (entity_name_normalized is_valid): %d rows", rows_sos
    )
...
def _check_smoke_target(con, *, bridge_run_id: str) -> bool:
    """Check that the smoke target resolves through the bridge to entity_num='3566305'.

    Validator-confirmed 2026-05-21 smoke target:
      '6Sense Insights, Inc.'
      → normalize_entity_name →
      '6sense insights'
      → CA SoS entity_num='3566305' (platinum tier).
    """
...
def _build_match_table(
    ...
    *,
    ...
    CA SoS side is joined directly — entity_name_normalized is already
    _lib-normalized (prediction p3 — do NOT re-normalize).
    ...
    #         by construction).
    #    SoS: sos.entity_name_normalized (already _lib-normalized at PR #464 —
    #         join directly; do NOT re-normalize per prediction p3).
    con.execute(
        f"""
    ...
    #    sos_fan_out: # of distinct CA SoS entity_nums for this normalized name.
...
def _write_bridge_lance(con, storage_options: dict) -> int:
    ...
    (CA SoS PK — directive Constraint; NOT ppp_legal_name_normalized
    per prediction p6). Dual BTREE per contract.
...
def main() -> int:
    """Build the SEC BDC SOI portfolio companies × CA SoS entities Pattern B bridge."""
    parser = argparse.ArgumentParser(
        description="SEC BDC SOI portfolio companies × CA SoS entities Pattern B bridge generator."
    )
    parser.add_argument(
        "--apply",
        action="store_true",
    ...
    logger.info(
        "bridge: %s  method=%s v%s (REUSED from PR #627, 1st REUSER)  normalizer=v%s  apply=%s",
    ...
    # REUSER: only register_bridge (no method-definition helpers per L21/p1).
    register_bridge(
        ...
        description=(
            ...
            "(sec_bdc_ppp publisher). First REUSER. "
            ...
            "CA SoS side: full ca_entities_lance (9.39M entity_name_normalized-valid rows), "
            "entity_name_normalized joined directly (already _lib-normalized, PR #464). "
            "No state filter — NAME-ONLY national match. "
            "BTREE on bdc_name_normalized + sos_entity_num. "
            "Floor: MIN_ROWS_MATCHED=4300 (validator-calibrated 2026-05-21; probe=6,153). "
            "Carries sos_entity_num + sos_entity_status from CA SoS."
        ),
    )
    ...
    try:
        ...
        # Hard-fail floor check — BEFORE Lance write (prediction p4).
