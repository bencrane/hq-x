#!/usr/bin/env python3
"""Project FMCSA Revocation Parquet → derived essentials Parquet on R2.

Reads `fmcsa/Revocation/<snapshot>/*.parquet.zst` via DuckDB-on-R2, projects
to snake_case column names (upstream uses spaces — "USDOT Number",
"Effective Date", etc.), writes plain `.parquet` to:

    s3://dex-raw-landing-zone/fmcsa-derived/revocation_essentials/snapshot=<YYYY-MM-DD>/data.parquet

The "USDOT Number" upstream column is renamed to `dot_number` to match the
canonical join-key name across the FMCSA spine.

Per-run audit row written to ops.fmcsa_derived_revocation_r2_ingest_runs.

L40 closure for FMCSA Revocation feed.
...
Usage:
    doppler run -p hq-all -c prd -- \\
        ...
        apps/data-engine-x/scripts/build_fmcsa_revocation_essentials.py --apply
"""
...
logger = logging.getLogger("build_fmcsa_revocation_essentials")

R2_BUCKET = "dex-raw-landing-zone"
INPUT_PREFIX = "fmcsa/Revocation"
OUTPUT_PREFIX = "fmcsa-derived/revocation_essentials"
AUDIT_TABLE = "ops.fmcsa_derived_revocation_r2_ingest_runs"

# (upstream_quoted_name, output_snake_case_name).
# Upstream Revocation Parquet has column names with SPACES — must be quoted.
COL_PROJECTIONS = [
    ('"USDOT Number"',                            "dot_number"),
    ('"Docket Number"',                           "docket_number"),
    ('"Operating Authority Registration Type"',   "registration_type"),
    ('"Serve Date"',                              "serve_date"),
            ...
            cur.execute(
                ...
                INSERT INTO {AUDIT_TABLE}
                  (snapshot_date, upstream_input_prefix, output_r2_prefix,
                   status, triggered_by)
                VALUES (%s, %s, %s, 'running', 'manual:build_fmcsa_revocation_essentials')
                RETURNING run_id;
                """,
