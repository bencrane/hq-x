"""IL IDFPR Roofing Contractors -> Lance emit (Pattern A, one-off operator-fired).

Reads the Illinois Dept of Financial & Professional Regulation (IDFPR) statewide
"Professional Licensing" registry (Socrata illinois-edp.data.socrata.com /
pzzh-kp68; 4.28M rows across 320 professions), FILTERED to the construction
trade Illinois licenses at the state level -- Roofing Contractor (Licensed +
Qualifying Party). IL does NOT issue a statewide general-contractor license
(GC is municipal), and the IDFPR omnibus is dominated by healthcare/cosmetology,
so we ingest only the roofing-contractor slice. Writes a Lance dataset at:
    s3://dex-raw-landing-zone/polaris-warehouse/licensure/il_idfpr_roofing_lance

R2 Parquet intermediate at:
    s3://dex-raw-landing-zone/il-idfpr-roofing/release={release}/data.parquet

Source (API-filtered):
    https://illinois-edp.data.socrata.com/resource/pzzh-kp68.csv
        ?$where=upper(description) like '%ROOFING CONTRACTOR%'
    33,377 rows confirmed 2026-05-29 (LICENSED 21,331 + QUALIFYING PARTY 12,046).

Columns:
  - 30 verbatim from CSV (all VARCHAR per L9 "emit verbatim").
  - business_name_normalized (additive, via normalize_entity_name UDF; business_name
    populated on ~100% of roofing rows, incl. qualifying-party rows).

BTREE indices on license_number + business_name_normalized + description
(LICENSED vs QUALIFYING PARTY ROOFING CONTRACTOR). Mirrors the other licensure
emits.
...
Run via:
    cd ~/hq-all/apps/data-engine-x && \\
      ...
      uv run python scripts/run_il_idfpr_roofing_lance_emit.py \\
        --csv-path "/tmp/il_roofing.csv" --release 2026-05-29
"""
...
DATASET_SLUG = "il_idfpr_roofing_lance"
LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/licensure/il_idfpr_roofing_lance"
MIN_ROW_FLOOR = 30_000  # probe parsed 33,377 on 2026-05-29
...
def emit(csv_path: str, release: str) -> dict:
    ...
    local_parquet = f"/tmp/il-idfpr-roofing-{release}.parquet"
    r2_key = f"il-idfpr-roofing/release={release}/data.parquet"
    ...
    con.execute(
        f"""
        COPY (
            SELECT *, py_normalize_entity("business_name") AS business_name_normalized
    ...
    for col in ("license_number", "business_name_normalized", "description"):
...
def main() -> None:
    parser = argparse.ArgumentParser(description="IL IDFPR Roofing Contractors -> Lance emit (one-off)")
