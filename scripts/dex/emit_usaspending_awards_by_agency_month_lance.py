#!/usr/bin/env python3
"""Emit usaspending/awards_by_agency_month_lance — ALL-years agency×month rollup.
...
(awarding_toptier_agency_name, action_month, kind) rollup.

Schema (one row per (agency_name, action_month, kind) tuple):
  awarding_toptier_agency_name  VARCHAR
...
NOTE: awards_lance has NO awarding_toptier_agency_id column (only
awarding_agency_id at sub-tier). Agency rollup MUST use
awarding_toptier_agency_name (string). (contract.md §awards schema schema-drift)

BTREE on: awarding_toptier_agency_name, action_month.

MIN_ROW_FLOOR: 17,000 (validator-stamped; ~22K rows expected for all years).

Usage:
  doppler run --project hq-all --config prd -- \\
    uv run python apps/data-engine-x/scripts/emit_usaspending_awards_by_agency_month_lance.py --apply

  doppler run --project hq-all --config prd -- \\
    uv run python apps/data-engine-x/scripts/emit_usaspending_awards_by_agency_month_lance.py --dry-run
"""
...
logger = logging.getLogger("emit_usaspending_awards_by_agency_month_lance")

DATASET_SLUG = "usaspending_awards_by_agency_month_lance"
OUT_LANCE_URI = (
    "s3://dex-raw-landing-zone/polaris-warehouse/usaspending/awards_by_agency_month_lance"
)
AWARDS_LANCE_URI = (
...
BTREE_COLS = ["awarding_toptier_agency_name", "action_month"]
...
def main() -> int:
    ...
    # kind discriminator from award type:
    #   contract: type IN ('A','B','C','D') — FAR contract types
    #   assistance: otherwise (FABS numeric codes)
    con.execute("""
