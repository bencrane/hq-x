"""s1 - UCC CA lender × CA SoS entities owner-identity bridge (Pattern B).

**v2.0.0 (address-axis composite):** mirrors the sibling debtor bridge —
joins on legal-name + state exact-equality AND checks whether the UCC
secured party's normalized physical address agrees with any of the SoS
entity's three address roles. Address agreement promotes the name-axis
fan-out tier (platinum requires both 1:1 name AND address corroboration).
See sibling [build_bridge_ucc_ca_debtor_sos_ca_owner_lance.py](build_bridge_ucc_ca_debtor_sos_ca_owner_lance.py)
docstring for the full composite rule + schema additions.

Pattern B exact-match bridge: UCC CA secured parties filtered to
``SECURED_PARTY_TYPE='Organization'`` (lender legal name from filings) ×
...
Method: ``legal_name_state_exact_ca_with_address_corroboration`` v1.0.0
(NEW method registered by this script — same method as the sibling debtor
bridge; idempotent UPSERT keeps the row in sync between the two).
...
MIN_ROWS_MATCHED = 14_929 (validator-calibrated 2026-05-16 post full-corpus
baseline probe — 0.5 × observed distinct-name overlap of 29,858).

Inputs:
    s3://dex-raw-landing-zone/polaris-warehouse/ucc_ca/secured_parties_lance
        (filter pc.field('SECURED_PARTY_TYPE') == 'Organization'; normalize
...
Output:
    s3://dex-raw-landing-zone/polaris-warehouse/bridges/ucc_ca_lender_sos_ca_owner_lance
    (BTREE on secured_party_name_normalized AND entity_num)
...
Match method REUSE (validator p4 / L21):
    ...
    fail_bridge_run (on shortfall)           → status=failed + error
...
Modal hosting (validator p2): @app.function(cpu=8, memory=32768, timeout=10800)
Memory 32 GB matches PR #464's empirically-validated shape; defeats the
...
Run via:
    cd ~/hq-all/apps/data-engine-x && \\
      ...
      modal run scripts/build_bridge_ucc_ca_lender_sos_ca_owner_lance.py::run
"""
...
app = modal.App("data-engine-x-ucc-ca-lender-sos-ca-owner-lance")
...
BRIDGE_NAME = "ucc_ca_lender_sos_ca_owner"
METHOD_NAME = "legal_name_state_exact_ca_with_address_corroboration"
METHOD_SEMVER = "1.0.0"
BRIDGE_VERSION = "2.0.0"

COLLISION_THRESHOLD = 50
# Validator-calibrated 2026-05-16 post full-corpus baseline probe.
# UCC CA Organization rows = 4,684,136; ucc_distinct_normalized = 89,759;
# sos_distinct_normalized = 8,373,104; observed distinct-name overlap = 29,858.
# Floor = 0.5 × 29,858 = 14,929. RISK-FLAG (below 50K nominal) but operator
# intent permits ship. Pattern B fan-out lifts actual rows_matched 2-5×.
MIN_ROWS_MATCHED = 14_929

DATASET_SLUG = "ucc_ca_lender_sos_ca_owner_lance"
...
def _materialize_inputs(storage_options: dict, normalize_entity_name) -> tuple:
    """Load UCC CA secured parties (Organization) + CA SoS entities into Arrow.

    UCC side:
      - Read raw ORG_NAME + identifying/address columns from secured_parties_lance,
        filter to SECURED_PARTY_TYPE='Organization' at the Lance scanner.
      - Normalize ORG_NAME in Python via _lib/entity_name_normalize
        (canonical normalizer; validator p1).
    ...
    logger.info(
        "  ucc secured_parties_lance (SECURED_PARTY_TYPE=Organization): %d rows",
        rows_ucc_raw,
    )

    # Normalize ORG_NAME in Python (canonical normalizer ONLY).
    ...
    mask = pc.is_valid(ucc_tbl.column("secured_party_name_normalized"))
    ucc_tbl = ucc_tbl.filter(mask)
    rows_ucc_post_norm = len(ucc_tbl)
    logger.info(
        "  ucc after normalization (secured_party_name_normalized is_valid): %d rows",
        rows_ucc_post_norm,
    )
...
def _build_match_table(
    ...
    *,
    ...
    # 1. Inner JOIN on normalized name; pull address-axis columns through.
    con.execute(
        f"""
        ...
        SELECT
            ...
            u.SECURED_PARTY_TYPE             AS secured_party_type,
            u.ORG_NAME                       AS secured_party_org_name_raw,
            u.secured_party_name_normalized,
            u.ADDR1                          AS secured_party_addr1,
            u.CITY                           AS secured_party_city,
            u.STATE                          AS secured_party_state,
            u.POSTAL_CODE                    AS secured_party_postal_code,
            u.address_base_normalized        AS secured_party_address_base_normalized,
            s.entity_num,
            s.entity_name,
            ...
            u.secured_party_name_normalized  AS match_value_normalized,
            'CA'                             AS match_state,
        ...
        FROM ucc u
        JOIN sos s
          ON u.secured_party_name_normalized = s.entity_name_normalized
        """
    )
    ...
    # 2. Fan-out counts.
    #    ucc_fan_out: # of UCC filings sharing this normalized name (row-level fan-out).
    #    sos_fan_out: # of distinct SoS entity_num values for this normalized name.
    con.execute(
        """
        CREATE TEMP TABLE ucc_fanout AS
        SELECT secured_party_name_normalized, COUNT(*) AS ucc_fan_out
    ...
    con.execute(
        ...
        SELECT secured_party_name_normalized,
               COUNT(DISTINCT entity_num) AS sos_fan_out
        FROM bridge_raw GROUP BY 1
        """
    )

    # 3. Tier rule + address agreement + composite tier (v2.0.0 — mirrors
    # ucc_ca_debtor_sos_ca_owner). See that script's docstring for rule details.
    con.execute(
        f"""
        ...
        SELECT
            ...
            sf.sos_fan_out,
            (
              b.secured_party_address_base_normalized IS NOT NULL
              AND (
                b.secured_party_address_base_normalized = b.sos_principal_address_base_normalized
                OR b.secured_party_address_base_normalized = b.sos_principal_address_in_ca_base_normalized
                OR b.secured_party_address_base_normalized = b.sos_mailing_address_base_normalized
              )
            )                                                    AS address_agrees,
            NULLIF(
              CONCAT_WS('|',
                CASE WHEN b.secured_party_address_base_normalized IS NOT NULL
                       AND b.secured_party_address_base_normalized = b.sos_principal_address_base_normalized
                     THEN 'principal' END,
                CASE WHEN b.secured_party_address_base_normalized IS NOT NULL
                       AND b.secured_party_address_base_normalized = b.sos_principal_address_in_ca_base_normalized
                     THEN 'principal_in_ca' END,
                CASE WHEN b.secured_party_address_base_normalized IS NOT NULL
                       AND b.secured_party_address_base_normalized = b.sos_mailing_address_base_normalized
                     THEN 'mailing' END
              ),
              ''
            )                                                    AS address_match_path,
            CASE
              WHEN b.secured_party_address_base_normalized IS NOT NULL
                   AND (
                     b.secured_party_address_base_normalized = b.sos_principal_address_base_normalized
                     OR b.secured_party_address_base_normalized = b.sos_principal_address_in_ca_base_normalized
                     OR b.secured_party_address_base_normalized = b.sos_mailing_address_base_normalized
                   )
              THEN b.secured_party_address_base_normalized
              ELSE NULL
            ...
            CASE
                WHEN uf.ucc_fan_out > {COLLISION_THRESHOLD}
                  OR sf.sos_fan_out > {COLLISION_THRESHOLD}
                    THEN 'rejected'
                WHEN uf.ucc_fan_out = 1 AND sf.sos_fan_out = 1
                    THEN 'platinum'
                WHEN uf.ucc_fan_out = 1 OR  sf.sos_fan_out = 1
                    THEN 'gold'
                ELSE 'silver'
            END                                                  AS name_confidence_tier
        FROM bridge_raw b
        JOIN ucc_fanout uf USING (secured_party_name_normalized)
        JOIN sos_fanout sf USING (secured_party_name_normalized)
        """
    )
...
def _write_bridge_lance(con, storage_options: dict, lance_commit_lock) -> int:
    ...
    with lance_commit_lock(DATASET_SLUG):
        ...
        try:
            ...
            logger.info("BTREE on secured_party_name_normalized: OK")
        except Exception as e:
            logger.error("BTREE on secured_party_name_normalized FAILED: %s", e)
            raise
        try:
            ...
            raise
        # v2.0.0 filter accelerators (non-fatal — these are not identity keys).
...
def emit() -> dict:
    """Build the UCC CA × CA SoS owner-identity bridge."""
    ...
    # its method-definition + version-definition rows here.
    from scripts._lib.match_method_registry import (
        complete_bridge_run,
        fail_bridge_run,
        register_bridge,
    ...
    logger.info("inputs: %s + %s", UCC_SP_LANCE_URI, SOS_ENTITIES_LANCE_URI)
    logger.info("output: %s", BRIDGE_LANCE_URI)

    # Provenance: register NEW method + version (v2.0.0 composite address-axis),
    # then register_bridge. Idempotent UPSERTs are shared with the sibling
    # bridge ucc_ca_debtor_sos_ca_owner — both use the same composite method.
    register_match_method(
    ...
    register_match_method_version(
        ...
        tier_rule_description=(
            ...
            "(stored in canonical `confidence_tier`): platinum REQUIRES name "
            "1:1 AND address_agrees; without address corroboration the name-"
            "axis platinum demotes to gold and gold demotes to silver."
        ),
        ...
        input_columns_left=[
            "secured_party_name_normalized", "STATE", "address_base_normalized",
        ],
        input_columns_right=[
            "entity_name_normalized",
            "principal_address_base_normalized",
        ...
        output_value_description=(
            "(secured_party_name_normalized, entity_num) pair with name-axis "
            "match + boolean address_agrees + address_match_path (which SoS "
            "address role(s) matched) + composite_confidence_tier."
        ),
    )
    register_bridge(
        ...
        description=(
            ...
            "v2.0.0 composite name+address axis. Same composite rule as the "
            "sibling debtor bridge. Joins UCC lender filings to CA-registered "
            "LLCs; promotes tier when the lender's normalized physical address "
