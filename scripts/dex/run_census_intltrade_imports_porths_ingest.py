#!/usr/bin/env python3
"""Census USA Trade Online — imports by port and HS commodity, monthly raw ingest.

Source:
    U.S. Census Bureau International Trade API.
    Endpoint: https://api.census.gov/data/timeseries/intltrade/imports/porths
    ...
    Variables: 28 total at this endpoint (verified via /variables.json).
                ...
                the broker mission is port-keyed, the full HS6 × CTY × PORT
                slab returns HTTP 500 (Census response-size cap), and the
                aggregate CTY codes (5XXX ASIA, 0020 USMCA, '-' TOTAL)
                complicate downstream consumption. A separate country-grain
                ingest can be added if needed.
...
Idempotency:
    INSERT ... ON CONFLICT (port, i_commodity, year, month) DO UPDATE WHERE
    row IS DISTINCT FROM EXCLUDED. Census revises the most-recent ~3 months
    on each release; revisions land cleanly via UPDATE.

Audit:
    ops.census_intltrade_imports_porths_ingest_runs — one row per invocation.

Coverage / volume:
    Default grain: HS6 (~250k rows per month nationwide, no CTY).
    Default rolling window: --months 24 (last 24 months ending in
    most-recent-published month). Use --start-year/--start-month and
    --end-year/--end-month for backfills.

    Per-month single request, response ~10 MB JSON.

Usage:
    PYTHONPATH=. doppler run -- python3 scripts/run_census_intltrade_imports_porths_ingest.py
    PYTHONPATH=. doppler run -- python3 scripts/run_census_intltrade_imports_porths_ingest.py --start-year 2020 --start-month 1
    PYTHONPATH=. doppler run -- python3 scripts/run_census_intltrade_imports_porths_ingest.py --comm-lvl HS2 --months 60
    PYTHONPATH=. doppler run -- python3 scripts/run_census_intltrade_imports_porths_ingest.py --dry-run
"""
...
PROVIDER = "census_intltrade_imports_porths"
USER_AGENT = "data-engine-x-api/census-intltrade-imports-porths-ingest"
ENDPOINT = "https://api.census.gov/data/timeseries/intltrade/imports/porths"
SOURCE_FILENAME = "data/timeseries/intltrade/imports/porths"

DEFAULT_COMM_LVL = "HS6"
DEFAULT_LOOKBACK_MONTHS = 24
# Census's typical publication lag from end-of-month is 6–7 weeks.
# 75 days conservatively skips months that aren't yet published.
DEFAULT_END_LAG_DAYS = 75
DB_BATCH_SIZE = 5_000

# Census variables we request. All are confirmed-exposed at imports/porths
# per https://api.census.gov/data/timeseries/intltrade/imports/porths/variables.json
# (28 total — we request 23, omitting CTY_CODE/CTY_NAME for port-rollup
# grain and `time` which is the filter param).
#
# COMM_LVL is also a filter param but Census echoes it per row; we surface it
# as a typed column for fidelity.
CENSUS_VARS: tuple[str, ...] = (
    "PORT", "PORT_NAME",
    "I_COMMODITY", "I_COMMODITY_LDESC", "I_COMMODITY_SDESC",
...
# Census var → DB col (lowercase verbatim).
...
# Typed columns the upsert binds. Includes the four PK cols + every var we
# request. cty_code / cty_name exist in the schema for forward-compat but
# are NOT requested — they stay NULL on this rollup-grain ingest.
TYPED_COLS: tuple[str, ...] = (
    "port", "i_commodity", "year", "month",
    "port_name",
    "i_commodity_ldesc", "i_commodity_sdesc",
    "comm_lvl", "summary_lvl", "summary_lvl2", "last_update",
    "gen_val_mo", "gen_val_yr",
    "air_val_mo", "air_val_yr",
    "ves_val_mo", "ves_val_yr",
...
PK_COLS: tuple[str, ...] = ("port", "i_commodity", "year", "month")
...
def _logger() -> logging.Logger:
    ...
    return logging.getLogger("census_intltrade_imports_porths_ingest")
...
def _coerce_row(
    ...
    """Returns (typed_row, raw_row_dict).

    Census's 2D format: each row is a list of strings aligned to `header`.
    We zip into a dict for raw_source_row preservation and fill the typed
    columns from the configured CENSUS_TO_DB mapping.
    """
...
def _upsert(
    ...
    *,
    ...
    table = "entities.source_census_intltrade_imports_porths"
    all_cols = (
        *TYPED_COLS,
...
def _start_run(
    ...
    with conn.cursor() as cur:
        cur.execute(
            "INSERT INTO ops.census_intltrade_imports_porths_ingest_runs "
...
def _finish_run(
    ...
    with conn.cursor() as cur:
        cur.execute(
            "UPDATE ops.census_intltrade_imports_porths_ingest_runs SET "
...
def _approx_latest_published(today: date, lag_days: int) -> tuple[int, int]:
    """First-of-month for `today - lag_days`."""
        ...
        with httpx.Client(
            ...
            for y, m in _months_in_range(start_y, start_m, end_y, end_m):
                ...
                for raw in data_rows:
                    ...
                    if not typed.get("port") or not typed.get("i_commodity"):
