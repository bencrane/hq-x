#!/usr/bin/env python3
"""Re-emit FMCSA inspections_recent (vehicle_inspection_essentials) as a Lance dataset on R2.

Mirrors the Wave 1 canary ``run_fmcsa_carrier_essentials_lance_emit.py`` exactly.

Heaviest of the 4 new cohorts (~8.18M rows). The derivations.yaml transform for
``fmcsa_vehicle_inspection_essentials`` already lowercases ``DOT_NUMBER`` at
derivation time, so no rename is needed in this emit step.

Source: R2 parquet at
  ``r2://dex-raw-landing-zone/fmcsa-derived/vehicle_inspection_essentials/snapshot=YYYY-MM-DD/data.parquet``
Output:
  ``s3://dex-raw-landing-zone/polaris-warehouse/fmcsa/inspections_recent_lance/``
...
-----
    ...
    doppler run --project hq-all --config prd -- \\
        ...
        apps/data-engine-x/scripts/run_fmcsa_inspections_recent_lance_emit.py --dry-run

    # Apply:
    doppler run --project hq-all --config prd -- \\
        uv run --with pylance python3 \\
        apps/data-engine-x/scripts/run_fmcsa_inspections_recent_lance_emit.py --apply
...
LOG = logging.getLogger("fmcsa-inspections-recent-lance-emit")
...
DATASET_SLUG = "fmcsa_inspections_recent_lance"  # commit-lock key
TMP_DIR = "/tmp/lance"
