#!/usr/bin/env python3
"""Apply USAspending + SAM source extension — adds 5 RW sources for R2
prefixes that aren't covered by existing source_usaspending_contracts or
source_sam_entities.

Sources are metadata-only catalog entries (CREATE SOURCE … FORMAT PLAIN
ENCODE PARQUET); no MVs, no eager hydration past `pg_class` visibility.
First downstream MV reads from these sources will trigger hydration —
that's directive #3+'s concern.

Pattern lifted from PR #249's apply_rw_source_wiring_sweep.py. Same
audit ledger (ops.rw_source_wiring_runs), same idempotency policy
(skip if source already in pg_class), same DuckDB-on-R2 introspection.
Per-source debug budget: 15 min wall-clock.

Per-directive (#2.5) invariants:
  ...
  - L6: smoke = `pg_class` row only, never count(*).
  - L8: R2 inventory ground-truth before introspection; fail-loud if 0.
  - L19: source extensions DO NOT replace existing sources.

Usage:
    doppler run --project hq-all --config prd -- \\
        uv run --with duckdb --with psycopg[binary] --with boto3 --with pyyaml \\
        --with pydantic python \\
        apps/data-engine-x/scripts/apply_usaspending_sam_source_extension.py \\
        --apply
...
def _logger() -> logging.Logger:
    ...
    return logging.getLogger("apply-usaspending-sam-source-extension")
...
def _glob_to_search_prefix(match_pattern: str) -> str:
    """Convert an s3 connector glob to a boto3-friendly literal prefix."""
    for ch in ("*", "?", "["):
...
def list_r2_keys(bucket: str, match_pattern: str) -> list[str]:
    """Return up to 5 matching R2 keys for the given glob.

    boto3 list_objects_v2 has no glob support; we strip everything
    from the first wildcard onward to get a literal prefix that's
    strictly broader than the glob, then filter via fnmatch.
    """
    import fnmatch
    s3 = boto3.client(
        "s3",
    ...
    *,
    ...
    try:
        if time.monotonic() - started > PER_SOURCE_BUDGET_SEC:
            raise TimeoutError("per-source budget exceeded before R2 list")
        ...
        if time.monotonic() - started > PER_SOURCE_BUDGET_SEC:
            raise TimeoutError("per-source budget exceeded before introspect")
        ...
        if time.monotonic() - started > PER_SOURCE_BUDGET_SEC:
            raise TimeoutError("per-source budget exceeded before apply")
        with rw.cursor() as cur:
            cur.execute(ddl)
        rw.commit()
        log.info("[%s] CREATE SOURCE applied", entry.name)

        # Smoke: pg_class only (L6).
...
def main() -> int:
    ...
    parser.add_argument(
        "--config",
        default="apps/data-engine-x/scripts/_config/usaspending_sam_source_extension.yaml",
        help="Path to source-extension config YAML",
    )
    ...
    log.info("config: bucket=%s sources=%d", cfg.bucket, len(cfg.sources))
