#!/usr/bin/env python3
"""Re-emit FMCSA insurance_history (inshist_essentials) as a Lance dataset on R2.

Mirrors the Wave 1 canary ``run_fmcsa_carrier_essentials_lance_emit.py`` exactly.

Reads the daily derived Parquet at
  ``r2://dex-raw-landing-zone/fmcsa-derived/inshist_essentials/snapshot=YYYY-MM-DD/data.parquet``
and overwrites the Lance dataset at
  ``s3://dex-raw-landing-zone/polaris-warehouse/fmcsa/insurance_history_lance/``

BTREE scalar index on ``dot_number`` for per-DOT random access.
...
-----
    ...
    doppler run --project hq-all --config prd -- \\
        ...
        apps/data-engine-x/scripts/run_fmcsa_insurance_history_lance_emit.py --dry-run

    # Apply:
    doppler run --project hq-all --config prd -- \\
        uv run --with pylance python3 \\
        apps/data-engine-x/scripts/run_fmcsa_insurance_history_lance_emit.py --apply
...
LOG = logging.getLogger("fmcsa-insurance-history-lance-emit")

R2_BUCKET = "dex-raw-landing-zone"
INPUT_PREFIX = "fmcsa-derived/inshist_essentials"
LANCE_URI = (
    "s3://dex-raw-landing-zone/polaris-warehouse/fmcsa/insurance_history_lance"
)
DATASET_SLUG = "fmcsa_insurance_history_lance"  # commit-lock key
TMP_DIR = "/tmp/lance"
