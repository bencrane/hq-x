#!/usr/bin/env python3
"""DuckDB bridge generator: UCC CO secured parties × GLEIF LEI records (Lance edition).

CO parity port of build_bridge_ucc_gleif_lance.py (the CA edition).

Reads:
  UCC:   polaris-warehouse/ucc_co/secured_parties_lance/  (2.06M rows, raw — no pre-normalized name)
  GLEIF: polaris-warehouse/gleif/lei_records_lance/        (3.3M rows)

ucc_co/secured_parties_lance carries the CA-parity column schema (UCC1_NUM,
SECURED_PARTY_TYPE, ORG_NAME, STATE, ...) per emit_ucc_co_secured_parties_lance.py,
so the bridge logic is identical to the CA edition — only the input URI differs.
...
method calls only register_bridge() — leaving the shared match_methods /
match_method_versions rows intact.

Output: polaris-warehouse/bridges/ucc_co_gleif_lance/
...
Usage:
  doppler run --project hq-all --config prd -- \\
    ...
    apps/data-engine-x/scripts/build_bridge_ucc_co_gleif_lance.py --apply
"""
...
logger = logging.getLogger("build_bridge_ucc_co_gleif_lance")

BRIDGE_NAME = "ucc_co_gleif"
...
BRIDGE_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/ucc_co_gleif_lance"
DATASET_SLUG = "ucc_co_gleif_lance"

COLLISION_THRESHOLD = 50
# Calibrated from --dry-run: natural matched-row count 114,337 (71,808 platinum
# + 42,529 gold). Floor set conservatively below it as a regression tripwire.
MIN_ROWS_MATCHED = 90_000
TMP_DIR = "/tmp/lance"
...
def _materialize_inputs(storage_options: dict) -> tuple:
    ...
    logger.info("opening ucc_co/secured_parties_lance via Arrow-bridge ...")
    ...
    logger.info("  ucc_co/secured_parties_lance (SECURED_PARTY_TYPE=Organization): %d rows", rows_left)
...
def _ensure_registry() -> None:
    ...
    register_bridge(
        ...
        description=(
            "UCC CO secured parties (org-type) × GLEIF LEI records. "
            "Inline-normalized UCC ORG_NAME joined to GLEIF legal_name_normalized + state. "
            "Identifies which CO-UCC-active secured parties are LEI-registered entities. "
            "CO parity port of the ucc_gleif (CA) bridge; reuses company_name_state_exact v1.0.0."
        ),
    )
...
def main() -> int:
    ...
    logger.info("inputs: UCC CO secured_parties_lance + GLEIF lei_records_lance (Arrow-bridge)")
