...
Reads the YAML config at scripts/_config/usaspending_audience_mvs.yaml — 10
audience MV specs (9 active + 1 deferred for Form 5500 × SBA bridge) — and:
...
Usage:
    doppler run -p hq-all -c prd -- \\
        ...
        apps/data-engine-x/scripts/apply_usaspending_audience_mvs_rw.py --dry-run

    doppler run -p hq-all -c prd -- \\
        uv run --with pyyaml --with psycopg[binary] python \\
        apps/data-engine-x/scripts/apply_usaspending_audience_mvs_rw.py --apply
...
logger = logging.getLogger("apply_usaspending_audience_mvs_rw")

REPO_ROOT = Path(__file__).resolve().parents[3]
YAML_PATH = (
    REPO_ROOT
    / "apps/data-engine-x/scripts/_config/usaspending_audience_mvs.yaml"
)
...
def _load_specs() -> list[dict]:
    ...
    if len(audiences) != 12:
        raise SystemExit(
            f"FAIL: expected 12 audience entries in YAML "
            f"got {len(audiences)}"
        )
    seen = set()
    for a in audiences:
        ...
        seen.add(name)
        if not name.startswith("mv_audience_fed_"):
            raise SystemExit(
                f"FAIL: mv_name '{name}' violates naming convention "
                "(must start with 'mv_audience_fed_')"
            )
    return audiences
...
def _preflight_rw() -> None:
    ...
    out = _rw_psql(
        "SELECT count(*) FROM public.mv_sam_usaspending_aggregated;",
        fetch=True,
    ).strip()
    n = int(out)
    if n < 1_000:
        raise SystemExit(
            f"FAIL: mv_sam_usaspending_aggregated has only {n} rows; "
            "expected >1K. Re-hydration in progress?"
        )
    logger.info("preflight: mv_sam_usaspending_aggregated = %s rows", f"{n:,}")

    out = _rw_psql(
        "SELECT relname FROM pg_class WHERE relname = 'mv_sam_pdl_firmographic';",
        fetch=True,
    ).strip()
    if "mv_sam_pdl_firmographic" not in out:
        raise SystemExit(
            "FAIL: mv_sam_pdl_firmographic not found in pg_class. "
            "Predecessor directive #3 must apply before #6."
        )
    logger.info("preflight: mv_sam_pdl_firmographic exists (may be hydrating)")
        ...
        for spec in specs:
            if spec.get("deferred"):
                _seed_spec_row(
                    ...
                    notes=spec.get("deferred_reason", "DEFERRED: see YAML header"),
                )
            else:
                _seed_spec_row(
