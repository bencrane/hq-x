#!/usr/bin/env bash
# Rollback harness for cycle `sec-dera-fsds-ingest` (2026-05-18).
#
...
# when opening the PR):
#   ~/hq-all/apps/data-engine-x/scripts/migration-checks/2026-05-18-sec-dera-fsds-ingest-rollback.sh
#
# Reverse-order rollback per directive phase graph:
#   mod1 → p4..p1 → e4..e1 → r1 → c7..c1 → m2 → m1
#
...
#   m2       optional DELETE 5 ops.data_sources   | YES (git revert merge-SHA)
#            rows (only with --allow-data-loss)
#   c1-c7    (none — code-only changes)           | YES (git revert merge-SHA)
#   r1       aws s3 rm sec-dera/fsds/ -recursive  | no
#   e1-e4    aws s3 rm <lance prefix> -recursive  | no
#   p1-p4    Polaris DELETE Generic Table         | no
...
#
# Usage:
#   ./2026-05-18-sec-dera-fsds-ingest-rollback.sh --surface mod1
#   ./2026-05-18-sec-dera-fsds-ingest-rollback.sh --all                # all 19 in reverse order
#   ./2026-05-18-sec-dera-fsds-ingest-rollback.sh --all --allow-data-loss
#       # required to DELETE the 5 ops.data_sources rows (m2 surface)
#   ./2026-05-18-sec-dera-fsds-ingest-rollback.sh --surface m2 --allow-data-loss
#
...
echo "==> Rolling back sec-dera-fsds-ingest (surface=${SURFACE_FILTER:-ALL} allow_data_loss=$ALLOW_DATA_LOSS)"
...
run_rollback "p4" "
  doppler run --project hq-all --config prd -- bash -c '$(_polaris_delete_table fsds_num_lance)'
"

run_rollback "p3" "
  doppler run --project hq-all --config prd -- bash -c '$(_polaris_delete_table fsds_pre_lance)'
"

run_rollback "p2" "
  doppler run --project hq-all --config prd -- bash -c '$(_polaris_delete_table fsds_tag_lance)'
"

run_rollback "p1" "
  doppler run --project hq-all --config prd -- bash -c '$(_polaris_delete_table fsds_sub_lance)'
"
...
run_rollback "e4" '
  doppler run --project hq-all --config prd -- bash -c "
    ...
    aws s3 rm s3://'"$R2_BUCKET"'/polaris-warehouse/sec_dera/fsds_num_lance/ --recursive --endpoint-url=\$R2_ENDPOINT
  "
'
...
run_rollback "e3" '
  doppler run --project hq-all --config prd -- bash -c "
    AWS_ACCESS_KEY_ID=\$R2_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY=\$R2_SECRET_ACCESS_KEY \
    aws s3 rm s3://'"$R2_BUCKET"'/polaris-warehouse/sec_dera/fsds_pre_lance/ --recursive --endpoint-url=\$R2_ENDPOINT
  "
'
...
run_rollback "e2" '
  doppler run --project hq-all --config prd -- bash -c "
    AWS_ACCESS_KEY_ID=\$R2_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY=\$R2_SECRET_ACCESS_KEY \
    aws s3 rm s3://'"$R2_BUCKET"'/polaris-warehouse/sec_dera/fsds_tag_lance/ --recursive --endpoint-url=\$R2_ENDPOINT
  "
'
...
run_rollback "e1" '
  doppler run --project hq-all --config prd -- bash -c "
    AWS_ACCESS_KEY_ID=\$R2_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY=\$R2_SECRET_ACCESS_KEY \
    aws s3 rm s3://'"$R2_BUCKET"'/polaris-warehouse/sec_dera/fsds_sub_lance/ --recursive --endpoint-url=\$R2_ENDPOINT
  "
'
...
# ====================================================================== #
# Wipes the entire sec-dera/fsds/ prefix. ops.sec_dera_fsds_r2_ingest_runs
# rows remain in Postgres as the audit trail of "what was here before rollback."

run_rollback "r1" '
  doppler run --project hq-all --config prd -- bash -c "
    AWS_ACCESS_KEY_ID=\$R2_ACCESS_KEY_ID AWS_SECRET_ACCESS_KEY=\$R2_SECRET_ACCESS_KEY \
    aws s3 rm s3://'"$R2_BUCKET"'/'"$R2_FSDS_PREFIX"'/ --recursive --endpoint-url=\$R2_ENDPOINT
  "
'
...
run_rollback "c7" '_emit_code_revert "c7" "apps/data-engine-x/app/services/lance_views.py"'
run_rollback "c6" '_emit_code_revert "c6" "apps/data-engine-x/scripts/run_sec_dera_fsds_num_lance_emit.py"'
run_rollback "c5" '_emit_code_revert "c5" "apps/data-engine-x/scripts/run_sec_dera_fsds_pre_lance_emit.py"'
run_rollback "c4" '_emit_code_revert "c4" "apps/data-engine-x/scripts/run_sec_dera_fsds_tag_lance_emit.py"'
run_rollback "c3" '_emit_code_revert "c3" "apps/data-engine-x/scripts/run_sec_dera_fsds_sub_lance_emit.py"'
run_rollback "c2" '_emit_code_revert "c2" "apps/data-engine-x/modal/sec_dera_fsds_app.py"'
run_rollback "c1" '_emit_code_revert "c1" "apps/data-engine-x/scripts/run_sec_dera_fsds_r2_ingest.py"'

# ====================================================================== #
# Phase 1 (reverse) — Migration rollback
# ====================================================================== #

# ── m2: optional DELETE of 5 ops.data_sources rows ────────────────────── #
# Requires --allow-data-loss. Otherwise prints code-revert advisory only.
run_rollback "m2" '
  if [[ "$ALLOW_DATA_LOSS" -eq 1 ]]; then
    echo "    Deleting 5 ops.data_sources rows (--allow-data-loss)"
    dex_psql_ddl "DELETE FROM ops.data_sources WHERE display_name LIKE '\''sec_dera.fsds_%_lance'\'' OR display_name = '\''sec-dera/fsds'\''"
  else
    echo "    SKIP: m2 DELETE requires --allow-data-loss. Code-only rollback advisory:"
    _emit_code_revert "m2" "apps/data-engine-x/supabase/migrations/*_sec_dera_fsds_data_source_catalog_5_rows.sql"
  fi
'

# ── m1: ops.sec_dera_fsds_r2_ingest_runs is forward-only ──────────────── #
...
run_rollback "m1" '_emit_code_revert "m1" "apps/data-engine-x/supabase/migrations/*_sec_dera_fsds_r2_ingest_runs.sql"'
