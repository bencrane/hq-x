#!/usr/bin/env bash
# Rollback harness for cycle `caltrans-ccop-active-ingest` (2026-05-19).
# Pattern: mirrors `ca-cal-eprocure-archived-ingest-rollback.sh`.
#
...
echo "==> Rolling back caltrans-ccop-active-ingest (repo=${REPO_FILTER:-all} surface=${SURFACE_FILTER:-all})"
if [[ -n "$MERGE_SHA" ]]; then
...
# ── m2, m1: same git revert (forward-only DB policy) ─────────────────── #
rollback_surface "m2" "bencrane/hq-all" '
  echo "  (m2 rolled back by the c6 git revert above. Table ops.caltrans_ccop_ingest_runs remains in prod; remove manually if required: DROP TABLE ops.caltrans_ccop_ingest_runs CASCADE)"
'
rollback_surface "m1" "bencrane/hq-all" '
  echo "  (m1 rolled back by the c6 git revert above. Catalog row remains in prod; remove manually if required: UPDATE ops.data_source_catalog SET is_active=FALSE WHERE source_slug='"'"'caltrans_ccop_active'"'"')"
'
