#!/usr/bin/env bash
# Rollback harness for /scope cycle hq-all-sam-entity-pocs-lance-emit.
#
# Authored by Stage 3.A audit subagent (2026-05-16 UTC). Mirrors
# hq-all-fl-sunbiz-quarterly-ingest-and-sba-bridge-rollback.sh (direct precedent — PR #467).
#
# Runs surface rollbacks in REVERSE order (s5 → s4 → s3 → s2 → s1). All surfaces
# are forward-only per apps/data-engine-x/supabase/migrations/README.md §"Policy"
# — for code + migration + endpoint surfaces, rollback is `git revert <merge-SHA>`
# on hq-all main (the `ON CONFLICT DO UPDATE` + `IF NOT EXISTS` make re-apply
# idempotent). For Polaris registration (s4), DELETE is idempotent
# (200/204/404 all acceptable). For Railway deploy (s5), redeploy the prior
# deployment id via the validator-fixed monitoring/rollback command pair.
#
# Note: NONE of the surfaces here are destructive — all writes are NEW data
# (new emit script, new Lance dataset, new ops row, new Polaris registration,
# new view entry). The rollback gate per /scope SKILL.md is satisfied for
# every surface.
#
# Accepts:
...
rollback_surface "s4" "hq-all" '
  _polaris_delete_table "sam_gov" "entity_pocs_lance" || true
'

# ── s3: lance_views.py — git revert (1 LanceView entry appended) ────── #
rollback_surface "s3" "hq-all" '
  echo "manual: git -C $HQ_ALL_ROOT revert <merge-SHA> (removes the sam_entity_pocs_lance_raw LanceView entry from apps/data-engine-x/app/services/lance_views.py)."
...
rollback_surface "s2" "hq-all" '
  echo "manual: git -C $HQ_ALL_ROOT revert <merge-SHA> (removes supabase/migrations/{ts}_sam_entity_pocs_data_source.sql)."
  echo "  Forward-only per migrations/README.md §\"Policy\". ON CONFLICT (display_name) DO UPDATE makes re-apply idempotent."
  echo "  Note: the ops.data_sources row is NOT auto-deleted on revert (forward-only DDL). To manually retire it:"
  echo "    UPDATE ops.data_sources SET status='"'"'retired'"'"', retired_at=NOW() WHERE display_name='"'"'sam_entity_pocs_lance'"'"';"
'

# ── s1: sam_entity_pocs_lance emit script — git revert + R2 cleanup ──── #
rollback_surface "s1" "hq-all" '
  echo "manual: git -C $HQ_ALL_ROOT revert <merge-SHA> (removes run_sam_entity_pocs_lance_emit.py)."
  _r2_rm_prefix "polaris-warehouse/sam_gov/entity_pocs_lance" || true
  echo "  NB: the upstream sam_gov/entities_lance dataset is intentionally NOT cleaned up — it is the source dataset and is shared with other consumers."
'
