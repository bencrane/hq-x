#!/usr/bin/env bash
# Rollback harness for cycle `ny-sos-active-corporations-ingest` (2026-05-19).
# Pattern: mirrors `ca-opsc-school-facility-funding-ingest-rollback.sh`.
#
# Forward-only migrations (per apps/data-engine-x/supabase/migrations/README.md
# §"Policy") — all 7 surfaces share `git revert <merge-SHA>` as the post-merge
# rollback shape. Pre-merge rollback is "don't merge." Modal app deploy (s1)
# additionally gets `modal app stop` to disable cron in advance of git-revert.
#
# Usage:
#   ./ny-sos-active-corporations-ingest-rollback.sh                           # all surfaces, pre-merge path
#   MERGE_SHA=abc123 ./ny-sos-active-corporations-ingest-rollback.sh          # post-merge: revert merge
#   ./ny-sos-active-corporations-ingest-rollback.sh --surface s1              # one surface
#   ./ny-sos-active-corporations-ingest-rollback.sh --repo bencrane/hq-all    # repo filter (trivial; single-repo)
...
echo "==> Rolling back ny-sos-active-corporations-ingest (repo=${REPO_FILTER:-all} surface=${SURFACE_FILTER:-all})"
if [[ -n "$MERGE_SHA" ]]; then
...
# Reverse-order rollback per directive §"Rollback harness":
#   s1 → stop Modal cron first (disable scheduled execution)
#   e1 → optional R2 Lance dataset delete
#   r1 → optional R2 raw prefix delete
#   c6/c4/c2/c1 → all rolled back by single git revert of merge SHA
#   m2/m1 → rolled back by same git revert; tables/catalog rows remain in prod
#           (forward-only — operator manually drops if required)
...
rollback_surface "m2" "bencrane/hq-all" '
  echo "  (m2 rolled back by the c6 git revert above. Table ops.ny_sos_active_corporations_ingest_runs remains in prod; remove manually if required: DROP TABLE ops.ny_sos_active_corporations_ingest_runs CASCADE)"
'
rollback_surface "m1" "bencrane/hq-all" '
  echo "  (m1 rolled back by the c6 git revert above. Catalog row remains in prod; remove manually if required: UPDATE ops.data_source_catalog SET is_active=FALSE WHERE source_slug='"'"'ny_sos_active_corporations'"'"')"
'
