#!/usr/bin/env bash
# Rollback harness for cycle `az-app-rfp-public-ingest` (2026-05-19).
...
echo "==> Rolling back az-app-rfp-public-ingest (repo=${REPO_FILTER:-all} surface=${SURFACE_FILTER:-all})"
if [[ -n "$MERGE_SHA" ]]; then
...
rollback_surface "m2" "bencrane/hq-all" '
  echo "  (m2 rolled back by the c6 git revert above. Table ops.az_app_rfp_public_ingest_runs remains in prod; remove manually if required: DROP TABLE ops.az_app_rfp_public_ingest_runs CASCADE)"
'
rollback_surface "m1" "bencrane/hq-all" '
  echo "  (m1 rolled back by the c6 git revert above. Catalog row remains in prod; remove manually if required: UPDATE ops.data_source_catalog SET is_active=FALSE WHERE source_slug='"'"'az_app_rfp_public'"'"')"
'
