#!/usr/bin/env bash
# Rollback harness for cycle `fdot-scoc-active-contracts-ingest` (2026-05-19).
# Pattern: mirrors `ca-opsc-school-facility-funding-ingest-rollback.sh` but
# with c2/s1 elided — this cycle is manual-only (no Modal, no Cron) per CSCR
# PR #553 precedent.
...
echo "==> Rolling back fdot-scoc-active-contracts-ingest (repo=${REPO_FILTER:-all} surface=${SURFACE_FILTER:-all})"
if [[ -n "$MERGE_SHA" ]]; then
...
rollback_surface "m2" "bencrane/hq-all" '
  echo "  (m2 rolled back by the c6 git revert above. Table ops.fdot_scoc_active_contracts_ingest_runs remains in prod; remove manually if required: DROP TABLE ops.fdot_scoc_active_contracts_ingest_runs CASCADE)"
'
rollback_surface "m1" "bencrane/hq-all" '
  echo "  (m1 rolled back by the c6 git revert above. Catalog row remains in prod; remove manually if required: UPDATE ops.data_source_catalog SET is_active=FALSE WHERE source_slug='"'"'fdot_scoc_active_contracts'"'"')"
'
