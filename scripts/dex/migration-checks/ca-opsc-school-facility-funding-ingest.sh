#!/usr/bin/env bash
# Verification harness for cycle `ca-opsc-school-facility-funding-ingest` (2026-05-19).
#
...
#
# Usage:
#   ./ca-opsc-school-facility-funding-ingest.sh                  # all surfaces (soft for r1/e1)
#   ./ca-opsc-school-facility-funding-ingest.sh --surface m1     # one surface
#   STRICT=1 ./ca-opsc-school-facility-funding-ingest.sh         # post-deploy: missing r1/e1 FAIL
...
echo "==> Verifying ca-opsc-school-facility-funding-ingest (repo=${REPO_FILTER:-all} surface=${SURFACE_FILTER:-all} strict=$STRICT)"
...
FLOOR_ROWS=10000        # 14,202 actual per 2026-05-19 probe; floor at 10K leaves ~30% slack
...
run_surface "m1" "bencrane/hq-all" '
  CATALOG_COUNT=$(dex_psql_query "SELECT count(*) FROM ops.data_source_catalog WHERE source_slug='\''opsc_school_facility_funding'\'' AND is_active = TRUE" | tr -d "[:space:]") &&
  [[ "$CATALOG_COUNT" = "1" ]]
'

# ── m2: ops.opsc_school_facility_funding_ingest_runs table + index + view ext ── #
run_surface "m2" "bencrane/hq-all" '
  TABLE_EXISTS=$(dex_psql_query "SELECT 1 FROM pg_tables WHERE schemaname='\''ops'\'' AND tablename='\''opsc_school_facility_funding_ingest_runs'\''" | tr -d "[:space:]") &&
  [[ "$TABLE_EXISTS" = "1" ]] &&
  IDX_EXISTS=$(dex_psql_query "SELECT 1 FROM pg_indexes WHERE schemaname='\''ops'\'' AND indexname='\''idx_opsc_school_facility_funding_ingest_runs_status_started'\''" | tr -d "[:space:]") &&
  [[ "$IDX_EXISTS" = "1" ]] &&
  VIEW_COUNT=$(dex_psql_query "SELECT count(*) FROM ops.data_source_catalog_status WHERE source_slug='\''opsc_school_facility_funding'\''" | tr -d "[:space:]") &&
  [[ "$VIEW_COUNT" = "1" ]]
'
...
# ── c1: scripts/run_opsc_school_facility_funding_to_r2.py ──────────── #
run_surface "c1" "bencrane/hq-all" '
  F="$APP_DIR/scripts/run_opsc_school_facility_funding_to_r2.py" &&
  test -f "$F" &&
  python3 -c "import ast; ast.parse(open(\"$F\").read())" &&
  grep -q "data.ca.gov"                              "$F" &&
  grep -q "opsc/school-facility-funding"             "$F" &&
  grep -q "snapshot="                                "$F" &&
  grep -qE "ZSTD|zstd"                               "$F" &&
  grep -qE "dtype=str|all_varchar|astype.*str"       "$F" &&
  grep -q "application/x-parquet"                    "$F" &&
  grep -q "ops.opsc_school_facility_funding_ingest_runs" "$F"
'

# ── c2: modal/opsc_school_facility_funding_app.py ──────────────────── #
run_surface "c2" "bencrane/hq-all" '
  F="$APP_DIR/modal/opsc_school_facility_funding_app.py" &&
  test -f "$F" &&
  python3 -c "import ast; ast.parse(open(\"$F\").read())" &&
  grep -q "data-engine-x-opsc-school-facility-funding"  "$F" &&
  grep -qE "Cron\("                                     "$F" &&
  grep -q "dex-db"                             "$F" &&
  grep -q "bulk-ingest-r2"                              "$F" &&
  grep -qE "run_opsc_school_facility_funding_to_r2|weekly_opsc_refresh" "$F"
'

# ── c4: scripts/run_opsc_school_facility_funding_lance_emit.py ─────── #
run_surface "c4" "bencrane/hq-all" '
  F="$APP_DIR/scripts/run_opsc_school_facility_funding_lance_emit.py" &&
  test -f "$F" &&
  python3 -c "import ast; ast.parse(open(\"$F\").read())" &&
  grep -q "opsc_school_facility_funding_lance"  "$F" &&
  grep -q "castate"                             "$F" &&
...
# ── r1: OPSC SFP R2 partition has ≥1 .parquet ───────────────────────── #
...
# ── e1: Lance dataset — floor 10000, BTREE on application_number ───── #
_E1_PY=$(_lance_check_inline "$LANCE_URI" "$FLOOR_ROWS" "application_number" "")
