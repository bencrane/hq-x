#!/usr/bin/env bash
# Rollback harness for cycle `usaspending-sos-ny-owner-bridge` (2026-05-19).
#
# REUSER framing (CRITICAL — validator-set 2026-05-19):
#   This cycle is the EIGHTH REUSER of legal_name_state_exact_ny v1.0.0.
#   The match_methods + match_method_versions rows for that method were
#   created by PR #513 on 2026-05-18 and serve 7 sibling bridges:
#     sba_ny_contracts, sba_ny_usaspending, sba_ny_sam, sba_ny_nyc_contracts,
#     sba_ny_mta, sba_ny_local_authority, sam_sos_ny_entities (PR #569).
#   ⚠ DO NOT DELETE those rows on rollback ⚠
#   Rollback removes ONLY the usaspending_sos_ny_owner bridge row + its
...
echo "==> Rolling back usaspending-sos-ny-owner-bridge (repo=${REPO_FILTER:-all} surface=${SURFACE_FILTER:-all})"
if [[ -n "$MERGE_SHA" ]]; then
...
MODAL_APP_NAME="data-engine-x-usaspending-sos-ny-owner-bridge"
...
# are NOT touched — they were published by PR #513 and serve 7 sibling
# bridges (sba_ny_contracts/usaspending/sam/nyc_contracts/mta/local_authority,
# sam_sos_ny_entities).
rollback_surface "e2" "bencrane/hq-all" '
  echo "  (DB cleanup for THIS bridge only — REUSER-safe; PR #513 method row left intact for 7 sibling bridges)" &&
  _dex_doppler "psql \"\$DEX_DB_URL_DIRECT\" -v ON_ERROR_STOP=1 -c \"DELETE FROM ops.bridge_generation_runs WHERE bridge_name='\''usaspending_sos_ny_owner'\'';\"" &&
  _dex_doppler "psql \"\$DEX_DB_URL_DIRECT\" -v ON_ERROR_STOP=1 -c \"DELETE FROM ops.bridges WHERE bridge_name='\''usaspending_sos_ny_owner'\'';\"" &&
  echo "  (NOT touching ops.match_methods or ops.match_method_versions — those rows serve 7 sibling bridges per validator finding p2)"
'
...
rollback_surface "m1" "bencrane/hq-all" '
  ...
  echo "     UPDATE ops.data_source_catalog SET is_active=FALSE WHERE source_slug='\''usaspending_sos_ny_owner'\'';" &&
  echo "   The view recreation in m1 reverts on revert — the new usaspending_sos_ny_owner branch disappears from data_source_catalog_status.)"
'
