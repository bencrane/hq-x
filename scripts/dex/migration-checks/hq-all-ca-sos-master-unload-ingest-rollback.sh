#!/usr/bin/env bash
# Rollback harness for /scope cycle hq-all-ca-sos-master-unload-ingest.
#
# Runs surface rollbacks in REVERSE order (s9 → s8 → s7 → s6 → s5 → s4 → s3 →
# s2 → s1). All surfaces are forward-only per
# apps/data-engine-x/supabase/migrations/README.md §"Policy" — for the code +
...
#
# Mirrors overture-sba-borrower-bridge-rollback.sh shape.
...
rollback_surface "s8" "hq-all" '
  _polaris_delete_table "bridges" "sba_sos_ca_owner_lance" || true
  _polaris_delete_table "sos"     "ca_agents_lance"        || true
  _polaris_delete_table "sos"     "ca_principals_lance"    || true
  _polaris_delete_table "sos"     "ca_entities_lance"      || true
'
...
rollback_surface "s6" "hq-all" '
  echo "manual: git -C $HQ_ALL_ROOT revert <merge-SHA> (removes supabase/migrations/{ts}_ca_sos_data_sources.sql)."
  ...
  echo "      '"'"'ca_sos_entities_lance'"'"','"'"'ca_sos_principals_lance'"'"',"
  echo "      '"'"'ca_sos_agents_lance'"'"','"'"'sba_sos_ca_owner_lance'"'"');"
'

# ── s5: SBA × SoS CA owner bridge — git revert + R2 cleanup ──────────── #
rollback_surface "s5" "hq-all" '
  echo "manual: git -C $HQ_ALL_ROOT revert <merge-SHA> (removes build_bridge_sba_sos_ca_owner_lance.py)."
  _r2_rm_prefix "polaris-warehouse/bridges/sba_sos_ca_owner_lance" || true
  echo "  ops.bridge_generation_runs rows stay (forensic record, forward-only)."
'

# ── s4: ca_agents_lance emit script — git revert + R2 cleanup ────────── #
rollback_surface "s4" "hq-all" '
  echo "manual: git -C $HQ_ALL_ROOT revert <merge-SHA> (removes run_ca_sos_agents_lance_emit.py)."
  _r2_rm_prefix "polaris-warehouse/sos/ca_agents_lance" || true
'

# ── s3: ca_principals_lance emit script — git revert + R2 cleanup ────── #
rollback_surface "s3" "hq-all" '
  echo "manual: git -C $HQ_ALL_ROOT revert <merge-SHA> (removes run_ca_sos_principals_lance_emit.py)."
  _r2_rm_prefix "polaris-warehouse/sos/ca_principals_lance" || true
'

# ── s2: ca_entities_lance emit script — git revert + R2 cleanup ──────── #
rollback_surface "s2" "hq-all" '
  echo "manual: git -C $HQ_ALL_ROOT revert <merge-SHA> (removes run_ca_sos_entities_lance_emit.py)."
  _r2_rm_prefix "polaris-warehouse/sos/ca_entities_lance" || true
'

# ── s1: R2 ZSTD Parquet landing — git revert + R2 cleanup ─────────────── #
rollback_surface "s1" "hq-all" '
  echo "manual: git -C $HQ_ALL_ROOT revert <merge-SHA> (removes run_ca_sos_master_unload_to_r2.py)."
  _r2_rm_prefix "sos-ca/release=2026-05-16" || true
'
