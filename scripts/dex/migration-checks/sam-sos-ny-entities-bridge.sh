#!/usr/bin/env bash
# Verification harness for cycle `sam-sos-ny-entities-bridge` (2026-05-19).
#
# Pattern B Lance bridge — SAM × NY SoS Active Corporations Pattern B Lance
# bridge. REUSER of legal_name_state_exact_ny v1.0.0 (publisher PR #513,
# 2026-05-18). SEVENTH REUSER (after sba_ny_contracts, sba_ny_usaspending,
# sba_ny_sam, sba_ny_nyc_contracts, sba_ny_mta, sba_ny_local_authority).
# Mirror of FL precedent PR #563 / CA precedent PR #560.
#
...
#
# Usage:
#   ./sam-sos-ny-entities-bridge.sh                  # all surfaces (r1/e1/e2/s1 soft)
#   ./sam-sos-ny-entities-bridge.sh --surface m1     # one surface only
#   STRICT=1 ./sam-sos-ny-entities-bridge.sh         # post-deploy: missing r1/e1/e2/s1 FAIL
#
# Worktree-aware invocation per L61 (mirror of NY ingest cycle / FDOT precedent):
#   HQ_ALL_ROOT=/Users/benjamincrane/hq-all/.claude/worktrees/competent-rubin-fcce42 \
#   STRICT=1 \
#     bash apps/data-engine-x/scripts/migration-checks/sam-sos-ny-entities-bridge.sh
...
echo "==> Verifying sam-sos-ny-entities-bridge (repo=${REPO_FILTER:-all} surface=${SURFACE_FILTER:-all} strict=$STRICT)"
...
FLOOR_ROWS=18000        # MIN_ROWS_MATCHED (validator-calibrated 2026-05-19; ~68% of 26,592 probe yield)
...
# View MUST include a new sam_sos_ny_entities branch.
run_surface "m1" "bencrane/hq-all" '
  CATALOG_COUNT=$(dex_psql_query "SELECT count(*) FROM ops.data_source_catalog WHERE source_slug='\''sam_sos_ny_entities'\'' AND is_active = TRUE" | tr -d "[:space:]") &&
  [[ "$CATALOG_COUNT" = "1" ]] &&
  ...
  [[ "$VIEW_EXISTS" = "1" ]] &&
  VIEW_BRANCH=$(dex_psql_query "SELECT count(*) FROM ops.data_source_catalog_status WHERE source_slug='\''sam_sos_ny_entities'\''" | tr -d "[:space:]") &&
  [[ "$VIEW_BRANCH" = "1" ]]
'
...
# ── c1: scripts/build_bridge_sam_sos_ny_entities_lance.py ────────────── #
...
run_surface "c1" "bencrane/hq-all" '
  F="$APP_DIR/scripts/build_bridge_sam_sos_ny_entities_lance.py" &&
  test -f "$F" &&
  python3 -c "import ast; ast.parse(open(\"$F\").read())" &&
  grep -q "legal_name_state_exact_ny"        "$F" &&
  grep -q "sam_sos_ny_entities"              "$F" &&
  ...
  grep -qE "MIN_ROWS_MATCHED *= *18_000|MIN_ROWS_MATCHED *= *18000" "$F" &&
...
# ── c2: modal/sam_sos_ny_entities_bridge_app.py ──────────────────────── #
# Modal app name + Cron schedule + secrets + delegates to c1 build script.
run_surface "c2" "bencrane/hq-all" '
  F="$APP_DIR/modal/sam_sos_ny_entities_bridge_app.py" &&
  test -f "$F" &&
  python3 -c "import ast; ast.parse(open(\"$F\").read())" &&
  grep -q "data-engine-x-sam-sos-ny-entities-bridge" "$F" &&
  grep -q "Cron("                                    "$F" &&
  grep -q "dex-db"                          "$F" &&
  grep -q "bulk-ingest-r2"                           "$F" &&
  grep -q "timeout="                                 "$F" &&
  grep -q "memory="                                  "$F" &&
  grep -q "build_bridge_sam_sos_ny_entities_lance"   "$F"
'
...
run_surface "c6" "bencrane/hq-all" '
  ...
  grep -q "sam_sos_ny_entities_lance" "$F"
'
...
# ── e1: Lance row floor ≥ 18,000 + dual-BTREE on sam_uei + sos_dos_id ─ #
_E1_PY=$(_lance_check_inline "$LANCE_URI" "$FLOOR_ROWS" "sam_uei" "sos_dos_id")
...
#   - ops.bridges has a NEW row for sam_sos_ny_entities (this cycle owns it)
...
run_surface_soft "e2" "bencrane/hq-all" '
  ...
  # SCHEMA NOTE (reviewer-corrected 2026-05-19): ops.match_method_versions
  ...
  BR_COUNT=$(dex_psql_query "SELECT count(*) FROM ops.bridges WHERE bridge_name='\''sam_sos_ny_entities'\''" | tr -d "[:space:]") &&
  [[ "$BR_COUNT" = "1" ]] &&
  # ≥1 completed bridge_generation_runs row
  RUN_COUNT=$(dex_psql_query "SELECT count(*) FROM ops.bridge_generation_runs WHERE bridge_name='\''sam_sos_ny_entities'\'' AND status='\''completed'\''" | tr -d "[:space:]") &&
  [[ "$RUN_COUNT" -ge 1 ]] &&
...
run_surface_soft "s1" "bencrane/hq-all" '
  doppler run --project hq-all --config prd -- bash -c "
    set -e
    modal app list --json | jq -e \".[] | select(.Description==\\\"data-engine-x-sam-sos-ny-entities-bridge\\\") | select(.State==\\\"deployed\\\")\" >/dev/null
  "
'
