#!/usr/bin/env python3
"""Project FMCSA Rejected Insurance Filings Parquet → derived essentials Parquet on R2.

Reads `fmcsa/Rejected - All With History/<snapshot>/*.parquet.zst` via
DuckDB-on-R2, projects to snake_case column names (upstream uses spaces +
parens — "Form Code (Insurance or Cancel)"), writes plain `.parquet`
(internal column ZSTD only) to:

    s3://dex-raw-landing-zone/fmcsa-derived/rejected_essentials/snapshot=<YYYY-MM-DD>/data.parquet

Source path note: reads from `<Feed> - All With History/` (the full-state
variant, ~15K rows) rather than `<Feed>/` (daily-delta variant). See
`build_fmcsa_actpendinsur_essentials.py` for the full rationale.

Per-run audit row written to ops.fmcsa_derived_rejected_r2_ingest_runs.

L40 closure for FMCSA Rejected feed.
...
Usage:
    doppler run -p hq-all -c prd -- \\
        ...
        apps/data-engine-x/scripts/build_fmcsa_rejected_essentials.py --apply
"""
...
logger = logging.getLogger("build_fmcsa_rejected_essentials")

R2_BUCKET = "dex-raw-landing-zone"
INPUT_PREFIX = "fmcsa/Rejected - All With History"
OUTPUT_PREFIX = "fmcsa-derived/rejected_essentials"
AUDIT_TABLE = "ops.fmcsa_derived_rejected_r2_ingest_runs"

# (upstream_quoted_name, output_snake_case_name).
COL_PROJECTIONS = [
    ('"USDOT Number"',                    "dot_number"),
    ('"Docket Number"',                   "docket_number"),
    ('"Form Code (Insurance or Cancel)"', "form_code_insurance_or_cancel"),
    ('"Insurance Type Description"',      "insurance_type_description"),
    ('"Policy Number"',                   "policy_number"),
    ('"Received Date"',                   "received_date"),
    ('"Insurance Class Code"',            "insurance_class_code"),
    ('"Insurance Type Code"',             "insurance_type_code"),
    ('"Underlying Limit Amount"',         "underlying_limit_amount"),
    ('"Maximum Coverage Amount"',         "maximum_coverage_amount"),
    ('"Rejected Date"',                   "rejected_date"),
    ('"Insurance Branch"',                "insurance_branch"),
            ...
            cur.execute(
                ...
                INSERT INTO {AUDIT_TABLE}
                  (snapshot_date, upstream_input_prefix, output_r2_prefix,
                   status, triggered_by)
                VALUES (%s, %s, %s, 'running', 'manual:build_fmcsa_rejected_essentials')
                RETURNING run_id;
                """,
