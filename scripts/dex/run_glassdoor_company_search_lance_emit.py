"""s6 - Glassdoor /company-search Lance emit (Pattern A, Modal-hosted).

Reads r2://dex-raw-landing-zone/glassdoor/company_search/snapshot=YYYY-MM-DD/
data.parquet via DuckDB-on-R2. Computes input_query_normalized,
name_normalized (inline _normalize_entity_sql, identical to
run_jsearch_jobs_lance_emit.py:115-146 for parity with PDL
legal_name_normalized) + website_normalized (inline _normalize_domain_sql per
build_bridge_sam_pdl_domain_lance.py:109-119 precedent). Writes Lance to
s3://dex-raw-landing-zone/polaris-warehouse/glassdoor/company_search_lance.

4 BTREE: glassdoor_company_id, input_query_normalized, name_normalized,
website_normalized.

Per-tuple (input_query, company_id) rows preserved faithfully (source ingest
invariant — raw stays raw).

Run via (DETACH IS MANDATORY per L47):
    cd ~/hq-all/apps/data-engine-x && \\
      doppler run --project hq-all --config prd -- \\
      modal run --detach scripts/run_glassdoor_company_search_lance_emit.py::run
"""
...
app = modal.App("data-engine-x-glassdoor-company-search-lance-emit")
...
DATASET_SLUG = "glassdoor_company_search_lance"
LANCE_URI = (
    "s3://dex-raw-landing-zone/polaris-warehouse/glassdoor/company_search_lance"
)
MIN_ROW_FLOOR = 14  # ceil(0.9 × 15 validator-stamped Postgres floor)
BTREE_COLUMNS = [
    "glassdoor_company_id", "input_query_normalized",
...
# Inline domain normalization — IDENTICAL to
# build_bridge_sam_pdl_domain_lance.py:109-119 / run_jsearch_jobs_lance_emit.py.
...
# Inline entity-name normalization — IDENTICAL to
# run_jsearch_jobs_lance_emit.py:119-146. REQUIRED PARITY: pdl.legal_name_normalized
# was computed via this exact function; downstream s7 name-fallback join
# compares glassdoor.name_normalized = pdl.legal_name_normalized so both
# sides MUST use the same normalization.
...
def emit() -> dict:
    ...
    src_parquet_uri = (
        f"r2://dex-raw-landing-zone/glassdoor/company_search/"
