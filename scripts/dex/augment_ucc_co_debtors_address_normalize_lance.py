#!/usr/bin/env python3
"""Augment `ucc_co/debtors_lance` with a normalized address column.

CO sibling of `augment_ucc_ca_debtors_address_normalize_lance.py` (PR #803).
Identical pattern — Lance `add_columns` + BatchUDF over ADDR1+ADDR2+ADDR3
via `_lib.address_normalize.normalize_address_street` (base form,
unit-stripped).

Why:
  Unblocks the CO bridge rebuilds at v2.0.0 — `ucc_co_sba_borrower_lance`,
  `ppp_ucc_co_debtor_lance`, `sam_ucc_co_debtor_lance` — so they can use
  the same composite address-axis pattern landed for CA in PR #805 / #806.

Usage:
  doppler run --project hq-all --config prd -- \\
    uv run --with pylance --with pyarrow python \\
    apps/data-engine-x/scripts/augment_ucc_co_debtors_address_normalize_lance.py --apply
"""
...
logger = logging.getLogger("augment_ucc_co_debtors_address_normalize_lance")
