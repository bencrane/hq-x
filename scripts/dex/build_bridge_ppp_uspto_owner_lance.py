#!/usr/bin/env python3
"""Bridge generator: SBA PPP borrowers × USPTO trademark owners.

Sibling of `sba_uspto_owner_lance` for the PPP universe. Joins:
...
Identity-resolution path: PPP borrower's `legal_name_normalized` + 2-letter US
...
Why this bridge:
  PPP borrowers exhibit MORE legal-LLC-vs-operating-brand divergence than
  SBA 7(a)/504 — PPP swept in every nail salon, food truck, and operating
  LLC under their paperwork holding-name, often with no public-facing
  brand surface in the SBA paperwork itself. The USPTO trademark axis
  systematically surfaces the operating brand for PPP borrowers with
  registered marks. Composes structurally with `ppp_overture_address_lance`
  (21.8M rows) — cross-validation between storefront identity (Overture)
  and registered IP identity (USPTO) on the same PPP borrower.

Output shape (per matched row):
  - PPP identity:    legal_name_normalized, borrname_sample, borrstate, borrzip
  - PPP loan rollup: total_ppp_loans, total_ppp_approval, max/min_approval_date,
...
Fan-out tiering:
  platinum = 1:1 on (ppp_identity, serial_no)
  gold     = 1:N or N:1
...
borrzip) — the natural key of `sba/ppp_borrowers_lance`.

Output: `polaris-warehouse/bridges/ppp_uspto_owner_lance/`
Audit:  ops.bridge_generation_runs (bridge_name='ppp_uspto_owner')
Floor:  ≥ 100,000 rows.

Usage:
  doppler run --project hq-all --config prd -- \\
    uv run --with duckdb --with pylance --with pyarrow --with "psycopg[binary]" python \\
    apps/data-engine-x/scripts/build_bridge_ppp_uspto_owner_lance.py --apply

  doppler run --project hq-all --config prd -- \\
    uv run --with duckdb --with pylance --with pyarrow --with "psycopg[binary]" python \\
    apps/data-engine-x/scripts/build_bridge_ppp_uspto_owner_lance.py --dry-run
"""
...
logger = logging.getLogger("build_bridge_ppp_uspto_owner_lance")

BRIDGE_NAME = "ppp_uspto_owner"
...
BRIDGE_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/ppp_uspto_owner_lance"
DATASET_SLUG = "ppp_uspto_owner_lance"
...
TMP_DIR = f"/tmp/lance/ppp_uspto_owner_{os.getpid()}"
...
def _materialize_inputs(storage_options: dict) -> tuple:
    """Load PPP borrowers + USPTO owner + USPTO case_file Arrow tables.

    All three sides are pre-normalized at emit time — `legal_name_normalized`
    on both PPP and USPTO owner uses the same canonical SQL v1.0.0 rule
    ...
    return ppp_arrow, owner_arrow, case_arrow, rows_ppp, rows_owner, rows_case
...
def _build_match_table(
    ...
    *,
    ...
    """JOIN PPP × case_file_owner × case_file + fan-out tiering.

    Step 1: PPP × case_file_owner on (legal_name_normalized, state)
    ...
    logger.info(
        "  registered: ppp=%d  owner=%d  case=%d",
    ...
    # Step 1: PPP × owner on (legal_name, state). Both `legal_name_normalized`
    # columns are pre-baked. PPP borrstate is uppercase 2-letter; owner_state
    ...
    con.execute(
        ...
        FROM ppp p
        JOIN owner o
          ON p.legal_name_normalized = o.legal_name_normalized
         AND UPPER(TRIM(p.borrstate)) = o.owner_state_normalized
        """
    )
    ...
    # Fan-out tiering: PPP identity = (legal_name, state, zip); USPTO identity = serial_no
    con.execute(
        """
            ...
            CASE
                WHEN pf.ppp_fan_out > {COLLISION_THRESHOLD}
                  OR uf.uspto_fan_out > {COLLISION_THRESHOLD}
                    THEN 'rejected'
                WHEN pf.ppp_fan_out = 1 AND uf.uspto_fan_out = 1
                    THEN 'platinum'
                WHEN pf.ppp_fan_out = 1 OR  uf.uspto_fan_out = 1
                    THEN 'gold'
                ELSE 'silver'
...
def _ensure_registry() -> None:
    ...
    register_bridge(
        ...
        description=(
            "SBA PPP borrowers × USPTO trademark owners — legal-name + state "
            "exact match, stitched to case_file mark text + registration "
            "metadata via serial_no. Surfaces the registered operating brand "
            "behind PPP borrowers. PPP universe exhibits more legal-LLC-vs-"
            "operating-brand divergence than SBA 7(a)/504, so the trademark "
            "axis is even higher-leverage here. Composes structurally with "
            "ppp_overture_address_lance for storefront + brand cross-validation."
        ),
    )
...
def main() -> int:
    ...
    logger.info(
        "inputs: sba/ppp_borrowers_lance + uspto/case_file_owner_lance + uspto/case_file_lance"
    )
    ...
    try:
        ppp_arrow, owner_arrow, case_arrow, rows_ppp, rows_owner, rows_case = (
