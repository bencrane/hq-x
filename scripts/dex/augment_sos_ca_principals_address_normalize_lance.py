#!/usr/bin/env python3
"""Augment `sos/ca_principals_lance` with a normalized address column.

Adds:
  - `address_base_normalized` — from `address1+address2+address3` via
    `_lib.address_normalize.normalize_address_street` (base form,
    unit-stripped).

Why:
  Principal-grain joins (sec_adv_principal × sos_ca_principal address axis,
  ucc_debtor × sos_ca_principal personal-address axis, etc.) need the same
  pre-baked normalized join key without paying the Python UDF cost on 18.67M
  principal rows each time.
...
Usage:
  doppler run --project hq-all --config prd -- \\
    ...
    apps/data-engine-x/scripts/augment_sos_ca_principals_address_normalize_lance.py --apply
"""
...
logger = logging.getLogger("augment_sos_ca_principals_address_normalize_lance")

LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/sos/ca_principals_lance"
DATASET_SLUG = "sos_ca_principals_lance"
NEW_COLS = ("address_base_normalized",)
READ_COLS = ["address1", "address2", "address3"]
...
def _batch_udf(batch):
    ...
    a1 = batch.column("address1").to_pylist()
    a2 = batch.column("address2").to_pylist()
    a3 = batch.column("address3").to_pylist()
...
def _emit(dry_run: bool) -> int:
    ...
    if dry_run:
        ...
        for r in sample.to_pylist():
            joined = join_address_lines(r["address1"], r["address2"], r["address3"])
            norm = normalize_address_street(joined)
            logger.info("  %r + %r + %r -> %r -> %r", r["address1"], r["address2"], r["address3"], joined, norm)
    ...
    with lance_commit_lock(DATASET_SLUG):
        ...
        @batch_udf(
            output_schema=pa.schema([pa.field("address_base_normalized", pa.string())])
        )
        def udf(batch):
        ...
        logger.info("coverage: address_base_normalized=%d/%d (%.1f%%)", nn, n, 100.0 * nn / max(1, n))
