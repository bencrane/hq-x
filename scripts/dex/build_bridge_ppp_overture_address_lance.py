#!/usr/bin/env python3
"""Bridge generator: SBA PPP borrowers × Overture US Place — address-keyed.

Joins PPP loan paperwork directly to Overture storefronts on a pure address
axis. Both sides arrive with the join token already baked in:

  - PPP:      `borrower_address_normalized` (99.6% coverage, 10.14M rows,
              normalized via `_lib.address_normalize.normalize_address_street`
              base form at emit time in `emit_ppp_borrowers_lance.py`)
  - Overture: `address_base_normalized` (96.4% coverage, 15.37M rows,
              same normalizer baked at emit time in
              `emit_overture_us_places_lance.py`)

No Python normalize pass at build time — both sides come pre-baked from
their respective emits. This is the symmetric DDL match the name path has
enjoyed since `name_normalized` was baked.

Inputs:
  PPP:      `polaris-warehouse/sba/ppp_borrowers_lance`           (~10.18M rows)
...
Fan-out tiering:
  platinum = 1:1 on (ppp_identity, place_id)
  gold     = 1:N or N:1
  silver   = N:M (fan-out ≤ 50 on each side)
  rejected = any fan-out > 50

Where `ppp_identity` is the composite (legal_name_normalized, borrstate,
borrzip) — the natural key of `ppp_borrowers_lance`.

Output: `polaris-warehouse/bridges/ppp_overture_address_lance/`
Audit:  ops.bridge_generation_runs (bridge_name='ppp_overture_address')
Floor:  ≥ 500,000 rows.

Why this bridge:
  PPP borrowers are an order of magnitude wider than the SAM-registered
  universe (~11M vs ~884K). The vast majority will never appear in a
  SAM-keyed bridge because they are not federal contractors. The
  address-direct path is the only way to attribute Overture places to
  the full PPP universe.

Usage:
  doppler run --project hq-all --config prd -- \\
    uv run --with duckdb --with pylance --with pyarrow --with "psycopg[binary]" python \\
    apps/data-engine-x/scripts/build_bridge_ppp_overture_address_lance.py --apply

  doppler run --project hq-all --config prd -- \\
    uv run --with duckdb --with pylance --with pyarrow --with "psycopg[binary]" python \\
    apps/data-engine-x/scripts/build_bridge_ppp_overture_address_lance.py --dry-run
"""
...
logger = logging.getLogger("build_bridge_ppp_overture_address_lance")

BRIDGE_NAME = "ppp_overture_address"
...
BRIDGE_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/ppp_overture_address_lance"
DATASET_SLUG = "ppp_overture_address_lance"
...
def _materialize_inputs(storage_options: dict) -> tuple:
    """Load PPP + Overture Arrow tables; both already pre-normalized."""
    import lance
    ...
    rows_ppp = len(ppp_arrow)
    logger.info("  ppp_borrowers_lance (post-filter): %d rows", rows_ppp)
    ...
    return ppp_arrow, overture_arrow, rows_ppp, rows_overture


def _build_match_table(ppp_arrow, overture_arrow, *, bridge_run_id: str, generated_at_iso: str):
    """JOIN on (address_base_normalized, state, zip5) + fan-out tiering.

    Both sides are pre-normalized — no UDF calls during the JOIN. The PPP
    ...
    used in `emit_ppp_borrowers_lance.py` side-scan.
    """
    import duckdb
    ...
    con.execute(
        ...
        SELECT
            p.borrower_address_normalized                     AS address_base_normalized,
            UPPER(TRIM(p.borrstate))                          AS match_state,
            {zip5_expr.format(col='p.borrzip')}               AS match_zip5,
        ...
        FROM ppp p
        JOIN overture o
          ON p.borrower_address_normalized = o.address_base_normalized
         AND UPPER(TRIM(p.borrstate))      = o.address_region
         AND {zip5_expr.format(col='p.borrzip')} = o.address_postcode_5
        """
    )
    rows_matched = con.execute("SELECT COUNT(*) FROM matched").fetchone()[0]
    logger.info("  matched (pre-tier): %d rows", rows_matched)

    # Fan-out tiering: PPP identity = (legal_name, state, zip), Overture identity = place_id
    con.execute(
        """
            ...
            CASE
                WHEN pf.ppp_fan_out > {COLLISION_THRESHOLD}
                  OR of_.overture_fan_out > {COLLISION_THRESHOLD}
                    THEN 'rejected'
                WHEN pf.ppp_fan_out = 1 AND of_.overture_fan_out = 1
                    THEN 'platinum'
                WHEN pf.ppp_fan_out = 1 OR  of_.overture_fan_out = 1
                    THEN 'gold'
                ELSE 'silver'
...
def _write_bridge_lance(con, storage_options: dict) -> int:
    ...
    with lance_commit_lock(DATASET_SLUG):
        ...
        for col in ("ppp_legal_name_normalized", "place_id", "address_base_normalized"):
            try:
...
def _ensure_registry() -> None:
    ...
    register_bridge(
        ...
        description=(
            "SBA PPP borrowers × Overture US Places — address-keyed exact "
            "match. Bypasses the legal-vs-public-name ceiling by joining on "
            "physical street address. PPP universe is ~10x wider than SAM "
            "and overwhelmingly uses operating addresses on loan paperwork, "
            "so this is the highest-yield single bridge for attaching "
            "Overture storefronts to the SBA capital-deployment universe."
        ),
    )
...
def main() -> int:
    ...
    logger.info(
        "inputs: sba/ppp_borrowers_lance + overture/us_places_lance (Arrow-bridge, pre-baked)"
    )
    ...
    try:
        ppp_arrow, overture_arrow, rows_left, rows_right = _materialize_inputs(
            storage_options
        )
        con, counts = _build_match_table(
            ppp_arrow, overture_arrow,
