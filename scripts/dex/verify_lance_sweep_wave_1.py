#!/usr/bin/env python3
"""Wave 1 Lance sweep — multi-source verification harness.

Runs the canary's verification gates (row-count parity, per-key random-
access latency) against each of the 4 sweep sources:

  1. fmcsa_crash_essentials_lance     (BTREE on dot_number)
  2. fmcsa_authhist_essentials_lance  (BTREE on dot_number)
  3. sam_gov_opps_active_lance        (BTREE on notice_id)
  4. usaspending_contracts_lance      (BTREE on recipient_uei)
...
Skips schema-evolution test (already validated by the canary; not gating
sweep). Skips the DuckDB Arrow round-trip and full-scan benches (also
validated by the canary).

Usage:
    doppler run --project hq-all --config prd -- \\
        uv run --with pylance --with duckdb --with requests \\
        --with boto3 --with psycopg python3 \\
        apps/data-engine-x/scripts/verify_lance_sweep_wave_1.py
...
LOG = logging.getLogger("verify-lance-sweep-wave-1")

ROW_COUNT_TOLERANCE_PCT = 0.1
PER_KEY_LATENCY_MS_BUDGET = 100.0
PER_KEY_SAMPLES = 20  # n=10 had too much flap; 20 stabilizes p50


@dataclass(frozen=True)
class SourceSpec:
    """Per-source config for the verifier."""
...
SOURCES: list[SourceSpec] = [
    SourceSpec(
        ...
        namespace="fmcsa",
        polaris_table="crash_essentials_lance",
        lance_uri="s3://dex-raw-landing-zone/polaris-warehouse/fmcsa/crash_essentials_lance",
        btree_column="dot_number",
        parquet_uri_for_count=(
            "LATEST_SNAPSHOT:r2://dex-raw-landing-zone/fmcsa-derived/crash_essentials"
        ),
    ),
    SourceSpec(
        display_name="fmcsa_authhist_essentials_lance",
        namespace="fmcsa",
        polaris_table="authhist_essentials_lance",
        lance_uri="s3://dex-raw-landing-zone/polaris-warehouse/fmcsa/authhist_essentials_lance",
        btree_column="dot_number",
        parquet_uri_for_count=(
            "LATEST_SNAPSHOT:r2://dex-raw-landing-zone/fmcsa-derived/authhist_essentials"
        ),
    ),
    SourceSpec(
        display_name="sam_gov_opps_active_lance",
        namespace="sam_gov",
        polaris_table="opps_active_lance",
        lance_uri="s3://dex-raw-landing-zone/polaris-warehouse/sam_gov/opps_active_lance",
        btree_column="notice_id",
        parquet_uri_for_count=(
            "LATEST_SNAPSHOT_GLOB:r2://dex-raw-landing-zone/sam-gov-opps/active"
        ),
    ),
    SourceSpec(
        display_name="usaspending_contracts_lance",
        namespace="usaspending",
        polaris_table="contracts_lance",
        lance_uri="s3://dex-raw-landing-zone/polaris-warehouse/usaspending/contracts_lance",
        btree_column="recipient_uei",
        parquet_uri_for_count=(
            "MULTI_YEAR:r2://dex-raw-landing-zone/usaspending/contracts:2024,2025,2026"
        ),
    ),
]
...
def _resolve_parquet_count(con, spec: SourceSpec) -> int:
    ...
    if uri_spec.startswith("LATEST_SNAPSHOT:"):
        prefix = uri_spec[len("LATEST_SNAPSHOT:"):]
        rows = con.execute(
            f"SELECT file FROM glob('{prefix}/snapshot=*/data.parquet')"
        ).fetchall()
        ...
        latest = snapshots[-1]
        uri = f"{prefix}/snapshot={latest}/data.parquet"
        return con.execute(
            f"SELECT COUNT(*) FROM read_parquet('{uri}')"
        ).fetchone()[0]

    if uri_spec.startswith("LATEST_SNAPSHOT_GLOB:"):
        prefix = uri_spec[len("LATEST_SNAPSHOT_GLOB:"):]
        rows = con.execute(
            f"SELECT file FROM glob('{prefix}/snapshot=*/*.parquet')"
        ).fetchall()
        snapshots = sorted(set(
            seg[len("snapshot="):]
            for (path,) in rows
        ...
        if not snapshots:
            raise SystemExit(f"FAIL: no snapshots under {prefix}")
        latest = snapshots[-1]
        uri_glob = f"{prefix}/snapshot={latest}/*.parquet"
        return con.execute(
            f"SELECT COUNT(*) FROM read_parquet('{uri_glob}')"
        ).fetchone()[0]

    if uri_spec.startswith("MULTI_YEAR:"):
        body = uri_spec[len("MULTI_YEAR:"):]
        prefix, _, years_csv = body.rpartition(":")
        years = [int(y) for y in years_csv.split(",")]
        uri_list = ", ".join(
            f"'{prefix}/year={y}/data.parquet'" for y in years
        )
        return con.execute(
...
def _bench_per_key_lookup(ds, btree_column: str, n: int) -> dict:
    # Sample keys from across the whole dataset by reading small windows
    # at several offsets. Two reasons:
    #   (a) Some datasets have millions of leading rows with NULL/empty
    #       keys (e.g. FMCSA crash_essentials).
    #   (b) Within a contiguous filter-scan window, Lance returns sequential
    #       rows which may share the same key value (e.g. all rows in the
    #       first 5000 of authhist belong to the same low DOT range). This
    #       gives the benchmark <5 unique keys instead of n=20.
    # The offset sweep guarantees key diversity for multi-row-per-key
    # cohorts.
    ...
    # Warm-up: cold-cache R2 access on the first index page can add 500ms+
    # of latency; throw away N untimed calls so subsequent measurements
    # reflect steady-state per-key lookup (the canary's 73ms result).
    # 3 warm-ups stabilizes p50 better than 1 — the BTREE index's first
    # several pages get cached on the local filesystem after Lance reads
    # them, then subsequent calls hit warm cache.
    ...
    for key in keys:
        # Quote with single quotes; assume the key value contains no quote
        # characters (FMCSA DOT, SAM notice_id, USAspending UEI all
        # satisfy this).
...
def main() -> int:
    ...
    ap.add_argument(
        ...
        default=None,
        help=(
            "Comma-separated subset of display_names to verify (default: all). "
            "e.g. fmcsa_crash_essentials_lance,sam_gov_opps_active_lance"
        ),
    )
    args = ap.parse_args()
    ...
    if args.json:
        ...
        LOG.info("=" * 60)
        LOG.info("=== Wave 1 Lance sweep — verification summary ===")
        LOG.info("=" * 60)
        for r in results:
            LOG.info(
