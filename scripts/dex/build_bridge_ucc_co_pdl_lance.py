#!/usr/bin/env python3
"""DuckDB bridge generator: UCC CO secured parties × PDL companies, two-path.

CO parity port of build_bridge_ucc_pdl_lance.py (the CA edition).

Two-path bridge (UNION ALL with explicit match_path):

  Path A (via_gleif_parent): UCC secured party → ucc_co_gleif_lance (LEI) →
...
Output: polaris-warehouse/bridges/ucc_co_pdl_lance/
...
Usage:
  doppler run --project hq-all --config prd -- \\
    ...
    apps/data-engine-x/scripts/build_bridge_ucc_co_pdl_lance.py --apply
"""
...
logger = logging.getLogger("build_bridge_ucc_co_pdl_lance")

BRIDGE_NAME = "ucc_co_pdl"
METHOD_NAME = "company_name_state_exact"
METHOD_SEMVER = "1.0.0"
BRIDGE_VERSION = "1.0.0"

SOURCE_LEFT = "ucc_co_secured_parties_lance"
SOURCE_RIGHT = "pdl_free_companies_lance"

UCC_SP_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/ucc_co/secured_parties_lance"
GLEIF_PARENT_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/gleif/lei_with_parent_lance"
UCC_GLEIF_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/ucc_co_gleif_lance"
PDL_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/pdl/free_companies_lance"
BRIDGE_LANCE_URI = "s3://dex-raw-landing-zone/polaris-warehouse/bridges/ucc_co_pdl_lance"
DATASET_SLUG = "ucc_co_pdl_lance"

COLLISION_THRESHOLD = 50
# Calibrated from --dry-run: natural matched-row count 5,236 (2,438 platinum
# + 512 gold + 2,286 silver; 200,500 collision-rejected — CO UCC liens
# concentrate on a few large lender parents, so the via-GLEIF-parent path
# fans out hard and the >50 tier-gate culls it). Floor set conservatively
# below the matched count as a regression tripwire.
...
def _materialize_inputs(storage_options: dict) -> tuple:
    ...
    # Pre-normalize in Python (avoids DuckDB regex spill of millions of rows)
    ...
    # appears in many UCC1 filings). The bridge resolves ENTITIES not
    # FILINGS, so joining on the deduped distinct (name, state) set is the
    # correct semantics AND avoids cartesian explosion in the downstream
    # hash-joins.
    branded_set: set = set()
    ...
    logger.info("opening bridges/ucc_co_gleif_lance ...")
    ...
    logger.info("  ucc_co_gleif_lance: %d rows", len(ug_arrow))
...
def _build_match_table(
    ...
    *,
    ...
    # OOM-resistant: ucc_branded is already deduped to distinct (name, state)
    # at Python-build time, so the hash side is small. Composite-key
    # (name, state) INNER JOIN gated by NOT EXISTS against ucc_gleif.
    con.execute("""
...
def _ensure_registry() -> None:
    # Method company_name_state_exact v1.0.0 is shared (registered by the CA
    # ucc_gleif bridge). Per DATA-FACTORY-ARCHITECTURE-PATTERNS.md, a bridge
    # reusing an existing method calls only register_bridge() — re-registering
    # the method version would ON CONFLICT-overwrite the shared config.
    register_bridge(
        ...
        description=(
            "UCC CO secured parties × PDL companies (GLEIF-parent-aware). "
            "Two-path: via_gleif_parent (UCC→GLEIF LEI→parent→PDL) and "
            "direct (name+state). match_path column discriminates paths. "
            "CO parity port of the ucc_pdl (CA) bridge; reuses company_name_state_exact v1.0.0."
        ),
    )
...
def main() -> int:
    ...
    logger.info("inputs: UCC CO secured_parties + lei_with_parent + ucc_co_gleif + PDL (Arrow-bridge)")
