"""Unit tests for the pure pieces of the campaigns hierarchy.
...
from app.services.campaigns import compute_scheduled_send_at
...
def test_scheduled_send_at_zero_offset_returns_campaign_start_midnight() -> None:
...
def test_scheduled_send_at_no_campaign_start_date_is_none() -> None:
    out = compute_scheduled_send_at(
...
@pytest.mark.parametrize(
    ("channel", "provider"), sorted(VALID_CHANNEL_PROVIDER_PAIRS)
)
...
def test_campaign_create_rejects_unknown_fields() -> None:
...
def test_channel_campaign_create_rejects_negative_offset() -> None:
    with pytest.raises(ValidationError):
        ChannelCampaignCreate(
            campaign_id=uuid4(),
            name="x",
...
def test_channel_campaign_update_allows_partial_payload() -> None:
    update = ChannelCampaignUpdate(name="renamed")
