...
3. [`docs/campaign-model.md`](docs/campaign-model.md) — the two-layer outreach model (current names). **This is the architectural anchor.** Everything in this directive flows from it.
...
8. [`app/services/channel_campaigns.py`](app/services/channel_campaigns.py) — `get_channel_campaign_context()` is what `emit_event` calls to resolve the six-tuple. Skim.
...
[#18](https://github.com/bencrane/hq-x/pull/18) shipped the primitive (originally `gtm_motions` + channel-typed `campaigns`), and [#22](https://github.com/bencrane/hq-x/pull/22) renamed it to its current shape: `business.campaigns` (umbrella) + `business.channel_campaigns` (channel-typed execution unit). The canonical six-tuple `(organization_id, brand_id, campaign_id, channel_campaign_id, channel, provider)` is enforced at every emit site by [`app/services/analytics.py::emit_event`](app/services/analytics.py).

That changes the calculus:

- The shape of "multi-channel analytics" is well-defined (campaign → child channel_campaigns by channel/provider) and matches the data model directly.
...
3. **Campaign analytics router** — new `/api/v1/analytics/*` endpoints scoped to organizations. Three feature groups:
   - **Campaign rollup** — given a `campaign_id` (umbrella), return per-channel and per-channel_campaign breakdowns of events, outcomes, costs.
   - **Channel-campaign analytics** — per-channel_campaign drilldown. Same shape across channels (voice/sms/email/direct_mail), with channel-specific extensions where the data warrants.
...
4. **Event schema in ClickHouse** — a single `events` table the `emit_event()` helper writes to (in addition to event-typed tables). Becomes the substrate for cross-channel campaign analytics.
...
- **Per-lead state across channels** — explicitly out of scope per [`docs/campaign-rename-pr-notes.md`](docs/campaign-rename-pr-notes.md). Don't build `lead_progress` analogues.
- **Cross-campaign analytics rollup** (multiple umbrella campaigns at once) — defer.
- **Frontend** — backend only.
- **Tightening `direct_mail_pieces.channel_campaign_id` / `campaign_id` to NOT NULL** — wait for an orphan-free real-data backfill; out of this directive's scope.
- **Sequence-step analytics** — OEX had `/api/analytics/campaigns/{id}/sequence-steps`. The hq-x model has no sequence-step concept (channel_campaigns are atomic; choreography is via `start_offset_days` between sibling channel_campaigns under the same campaign). Don't port. If a future feature wants intra-channel-campaign step granularity, it's a separate design.
...
ORDER BY (organization_id, campaign_id, occurred_at, event_id)
```
...
- **Campaign rollup** — Postgres can answer the question by joining `call_logs` / `sms_messages` / `direct_mail_pieces` on `channel_campaign_id` for each child of the umbrella campaign. Implement the fallback. It's slower but correct.
...
**Note on `direct_mail_pieces`:** this row carries BOTH `channel_campaign_id` (execution) and `campaign_id` (umbrella) — see [`docs/campaign-rename-pr-notes.md`](docs/campaign-rename-pr-notes.md) §"Voice tables don't get a denormalized umbrella `campaign_id`". Voice/SMS rows resolve umbrella via the FK chain (`call_logs.channel_campaign_id` → `channel_campaigns.campaign_id`). For campaign-rollup queries hitting voice/SMS data, you must join through `business.channel_campaigns`. For direct-mail you can filter directly on `direct_mail_pieces.campaign_id`.
...
- Don't break `emit_event`'s current signature (`channel_campaign_id=...`). The RudderStack call sits inside the existing function, after `resolve_channel_campaign_context()` has run.
...
**PR title:** "Campaign analytics router + reliability + direct-mail funnels"
...
ClickHouse path: one query against the wide `events` table grouping by `(channel, provider, channel_campaign_id)` plus an `outcome` column derived from `event_name`. Use ClickHouse-native parameterization for `campaign_id`, `from`, `to`.

Postgres fallback: union over `call_logs`, `sms_messages`, `direct_mail_pieces`, joined to `business.channel_campaigns` on `channel_campaign_id` filtered by `campaign_id`. For `direct_mail_pieces` you can filter directly on its `campaign_id` column without the join (it carries the umbrella id denormalized — see §3.4). Aggregate in SQL or in Python (your call — measure first).

#### 3.3 Channel-campaign analytics endpoint

`GET /api/v1/analytics/channel-campaigns/{channel_campaign_id}/summary?from=&to=`

Same shape as campaign rollup but scoped to one channel_campaign. Channel-specific extensions:
...
`GET /api/v1/analytics/direct-mail?brand_id=&channel_campaign_id=&from=&to=`
...
- `tests/test_campaign_analytics_pure.py` — pure aggregation functions (group/sum/derive). Reference pattern: `tests/test_campaigns_pure.py`.
- `tests/test_campaign_analytics_db_fake.py` — service-level with `get_db_connection` in-memory fake. Reference pattern: `tests/test_campaigns_services_db_fake.py`.
- `tests/test_campaign_analytics_clickhouse.py` — ClickHouse path with mocked `ch_query`. Verify the SQL parameters and the result-shape mapping.
...
- Cross-org leakage tests: a request with org A's auth, asking about org B's campaign, returns 404. **Add this test for every new endpoint.** This is the single highest-value safety check.
...
1. **Six-tuple is sacred.** Every event written via `emit_event()` carries `(organization_id, brand_id, campaign_id, channel_campaign_id, channel, provider)`. Don't add a code path that bypasses `emit_event()`. Don't accept `organization_id` as a request body field on any analytics endpoint — it comes from auth context.
...
5. **No stepwise/lead-progress concepts.** The OEX model has `step_order`, `step_status`, `campaign_lead_progress`, `campaign_sequence_steps`. None of these exist in hq-x and none should be reintroduced under a new name. The unit of analytics is the channel_campaign within a campaign, not a step within a channel_campaign.
...
9. **Mind the umbrella-vs-execution naming.** `campaign_id` always = umbrella (`business.campaigns`). `channel_campaign_id` always = execution (`business.channel_campaigns`). Mixing these up will silently produce wrong rollups. When in doubt, re-read [`docs/campaign-model.md`](docs/campaign-model.md) §"Hierarchy".

10. **No frontend, no docs site.** Backend only. Frontend is a separate workstream.

11. **Migration discipline.** ClickHouse DDL is operator-applied; ship as `docs/clickhouse-schema.md` or `scripts/clickhouse/`, NOT as a numbered Postgres migration. hq-x Postgres migrations are sequenced (next is `0023_…`); only add one if you need a Postgres schema change. As of this directive, you don't.
...
| 3 | `app/services/campaign_analytics.py` | New | +250 |
...
| 3 | `tests/test_campaign_analytics_pure.py` | New | +200 |
| 3 | `tests/test_campaign_analytics_db_fake.py` | New | +400 |
| 3 | `tests/test_campaign_analytics_clickhouse.py` | New | +200 |
...
- `org_id` / `company_id` / `company_campaign_id` field names — use `organization_id` / `brand_id` / `partner_id` / `channel_campaign_id` per the hq-x model.
...
| Six-tuple gets bypassed by some new emit site | Code review enforces it. Consider a lint rule (grep gate in CI) that flags direct `clickhouse.insert_row` calls outside `app/services/analytics.py`. |
| Confusing the umbrella `campaign_id` with the execution `channel_campaign_id` in queries or response shapes | Hard-rule §5.9. When writing SQL, name your parameters explicitly (`umbrella_campaign_id`, `exec_channel_campaign_id`) until the rename is reflexive. Test fixtures should use distinguishable UUIDs (e.g. `00…01` vs `00…02`) so a swapped argument fails loudly. |
...
- [ ] `campaign_id` (umbrella) and `channel_campaign_id` (execution) are not confused in any query, response shape, or test fixture.
...
5. **Phase ordering** — the directive sequences `Phase 1 → 2 → 3` because it's the lowest-risk path. If you'd rather front-load RudderStack (Phase 2 first), nothing breaks. ClickHouse query helper (Phase 1) must precede campaign analytics (Phase 3).

---

**Author:** Drafted by the audit agent that produced [`AUDIT_RUDDERSTACK_CLICKHOUSE_PORT.md`](AUDIT_RUDDERSTACK_CLICKHOUSE_PORT.md) and [`AUDIT_CLICKHOUSE_QUERY_AND_MULTICHANNEL_ANALYTICS.md`](AUDIT_CLICKHOUSE_QUERY_AND_MULTICHANNEL_ANALYTICS.md). Updated post-rename ([#22](https://github.com/bencrane/hq-x/pull/22)) to use the current `campaigns` / `channel_campaigns` ontology. All file:line citations were live at draft time; verify before relying on any specific reference.
