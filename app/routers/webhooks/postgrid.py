"""PostGrid Print & Mail webhook receiver.

Path: POST /webhooks/postgrid

Flow per event:
  1. Verify HMAC-SHA256 signature (x-webhook-secret header)
...
from app.webhooks.postgrid_normalization import compute_postgrid_event_key
from app.webhooks.postgrid_processor import project_postgrid_event
from app.webhooks.postgrid_signature import validate_postgrid_payload_schema, verify_postgrid_signature
...
@router.post("/postgrid")
async def receive_postgrid_webhook(request: Request) -> JSONResponse:
    raw_body = await request.body()
    request_id = _request_id(request)

    incr_metric("webhook.events.received", provider_slug="postgrid")

    sig_result = verify_postgrid_signature(
        raw_body=raw_body, request=request, request_id=request_id
    )

    try:
        payload_any = json.loads(raw_body.decode("utf-8"))
    except (json.JSONDecodeError, UnicodeDecodeError) as exc:
        incr_metric("webhook.events.rejected", provider_slug="postgrid", reason="malformed_body")
        raise HTTPException(
    ...
    if not isinstance(payload_any, dict):
        incr_metric(
            "webhook.events.rejected", provider_slug="postgrid", reason="payload_not_object"
        )
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail={
    ...
    try:
        schema_version, identity = validate_postgrid_payload_schema(payload)
    except ValueError as exc:
        reason = str(exc)
        incr_metric(
            "webhook.events.rejected", provider_slug="postgrid", reason=reason.split(":", 1)[0]
        )
        log_event(
            "postgrid_webhook_schema_invalid",
    ...
    event_key = compute_postgrid_event_key(payload, raw_body)
    ...
    if not inserted:
        incr_metric("webhook.duplicate_ignored", provider_slug="postgrid")
        log_event(
            "postgrid_webhook_duplicate_ignored",
            request_id=request_id,
            event_key=event_key,
            event_type=event_type,
        )
    ...
    incr_metric("webhook.events.accepted", provider_slug="postgrid")

    try:
        projection = await project_postgrid_event(
            payload=payload, event_id=identity["event_id"]
        )
        ...
        log_event(
            "postgrid_webhook_projection_failed",
            level=logging.ERROR,
            request_id=request_id,
            event_key=event_key,
            error=str(exc)[:500],
...
@router.post("/postgrid/replay/{event_id}")
async def replay_postgrid_event(
    event_id: UUID,
    ...
    async with get_db_connection() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                ...
                FROM webhook_events
                WHERE id = %s AND provider_slug = 'postgrid'
                """,
                (str(event_id),),
            )
            row = await cur.fetchone()
    ...
    source_event_id = (
        replay_payload.get("id") or replay_payload.get("event_id") or str(event_id)
    )

    try:
        projection = await project_postgrid_event(
            payload=replay_payload, event_id=str(source_event_id)
        )
    except Exception as exc:  # noqa: BLE001
        async with get_db_connection() as conn:
            async with conn.cursor() as cur:
        ...
        incr_metric("webhook.replay_failed", provider_slug="postgrid")
        return JSONResponse(
    ...
    incr_metric("webhook.replay_processed", provider_slug="postgrid")
