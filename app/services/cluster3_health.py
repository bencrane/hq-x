...
async def overall_snapshot() -> dict[str, Any]:
    ...
    statuses = [
        ...
        snap["dispatch_last_hour"]["status"],
        snap["stuck_queue"]["status"],
        snap["unclassified_backlog"]["status"],
        snap["pending_review"]["status"],
        snap["reconciliation"]["status"],
        snap["source_material_freshness"]["status"],
    ]
    if "red" in statuses:
        overall = "red"
